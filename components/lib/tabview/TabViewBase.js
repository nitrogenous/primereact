...
const classes = {
	...
	panelcontainer: ({ props }) =>
		classNames("p-tabview-panels", props.panelContainerClassName),
	...
	tab: {
		header: ({ selected, disabled, headerClassName, _className }) =>
			classNames(
				"p-unselectable-text",
				{ "p-tabview-selected p-highlight": selected, "p-disabled": disabled },
				headerClassName,
				_className,
			),
		...
		content: ({
			...
			shouldUseTab(tab, index) && (!props.renderActiveOnly || isSelected(index))
				? classNames(
...
export const TabPanelBase = ComponentBase.extend({
	...
	getCProp: (tab, name) =>
		ObjectUtils.getComponentProp(tab, name, TabPanelBase.defaultProps),
	getCProps: (tab) =>
		ObjectUtils.getComponentProps(tab, TabPanelBase.defaultProps),
	getCOtherProps: (tab) =>
		ObjectUtils.getComponentDiffProps(tab, TabPanelBase.defaultProps),
});
