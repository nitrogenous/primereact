...
export const TabView = React.forwardRef((inProps, ref) => {
    ...
    const [backwardIsDisabledState, setBackwardIsDisabledState] =
        React.useState(true);
    const [forwardIsDisabledState, setForwardIsDisabledState] =
    ...
    const getTabPT = (tab, key, index) => {
        ...
        return mergeProps(
            ptm(`tab.${key}`, { tab: tabMetaData }),
            ptm(`tabpanel.${key}`, { tabpanel: tabMetaData }),
            ptm(`tabpanel.${key}`, tabMetaData),
            ptmo(getTabProp(tab, "pt"), key, tabMetaData),
        );
    };
    ...
    const shouldUseTab = (tab) => {
        return (
            tab &&
            ObjectUtils.isValidChild(tab, "TabPanel") &&
            hiddenTabsState.every((_i) => _i !== tab.key)
        );
    };
    ...
    const findVisibleActiveTab = (i) => {
        ...
        return (
            tabsInfo.find(
                ({ tab, index }) => !getTabProp(tab, "disabled") && index >= i,
            ) ||
            tabsInfo
                .reverse()
                .find(
                    ({ tab, index }) =>
                        !getTabProp(tab, "disabled") && i > index,
                )
        );
    };
    ...
    const onTabHeaderClose = (event, index) => {
        ...
        if (
            onBeforeTabClose &&
            onBeforeTabClose({ originalEvent: event, index }) === false
        ) {
            return;
        }
    ...
    const changeActiveIndex = (event, tab, index) => {
        ...
        if (!getTabProp(tab, "disabled")) {
            // give caller a chance to stop the selection
            if (
                props.onBeforeTabChange &&
                props.onBeforeTabChange({ originalEvent: event, index }) ===
                    false
            ) {
                return;
            }
    ...
    const onTabArrowRightKey = (event) => {
        const nextHeaderAction = findNextHeaderAction(
    ...
    const onTabArrowLeftKey = (event) => {
        const prevHeaderAction = findPrevHeaderAction(
            event.target.parentElement,
        );

        prevHeaderAction
            ? changeFocusedTab(prevHeaderAction)
            : onTabEndKey(event);
        event.preventDefault();
    };
    ...
    const getVisibleButtonWidths = () => {
        return [prevBtnRef.current, nextBtnRef.current].reduce(
    ...
    const navBackward = () => {
        const width =
            DomHandler.getWidth(contentRef.current) - getVisibleButtonWidths();
    ...
    useUpdateEffect(() => {
        if (ObjectUtils.isNotEmpty(hiddenTabsState)) {
            const tabInfo = findVisibleActiveTab(
                hiddenTabsState[hiddenTabsState.length - 1],
            );
    ...
    const createTabHeader = (tab, index) => {
        ...
        const rightIconElement =
            rightIcon && IconUtils.getJSXIcon(rightIcon, undefined, { props });
        ...
        const closableIconElement = closable
    ...
    const createContent = () => {
        ...
        const contents = React.Children.map(props.children, (tab, index) => {
            if (
                ...
                return (
                    <div {...contentProps}>
                        {!props.renderActiveOnly
                            ? getTabProp(tab, "children")
                            : selected && getTabProp(tab, "children")}
                    </div>
                );
            }
        });
    ...
    const createPrevButton = () => {
        ...
        const leftIcon = IconUtils.getJSXIcon(
            icon,
            { ...prevIconProps },
            { props },
        );
    ...
    const createNextButton = () => {
        ...
        const icon = props.nextButton || (
            <ChevronRightIcon {...nextIconProps} />
        );
        const rightIcon = IconUtils.getJSXIcon(
            icon,
            { ...nextIconProps },
            { props },
        );
