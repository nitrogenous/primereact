...
export const DataTable = React.forwardRef((inProps, ref) => {
    ...
    const isVirtualScrollerDisabled = () => {
        return (
            ObjectUtils.isEmpty(props.virtualScrollerOptions) ||
            !props.scrollable
        );
    };
    ...
    const restoreColumnWidths = () => {
        if (columnWidthsState.current) {
            ...
            if (
                props.columnResizeMode === "expand" &&
                tableWidthState.current
            ) {
    ...
    const getSelectionModeInColumn = (columns) => {
        if (columns) {
            const col = columns.find(
                (c) => !!getColumnProp(c, "selectionMode"),
            );
    ...
    const onColumnResizeEnd = () => {
        const delta =
            resizeHelperRef.current.offsetLeft - lastResizeHelperX.current;
        ...
        if (columnWidth + delta > parseInt(minWidth, 10)) {
            if (props.columnResizeMode === "fit") {
                const nextColumn =
                    resizeColumnElement.current.nextElementSibling;
    ...
    const onColumnHeaderDragOver = (e) => {
        ...
        if (
            ...
            if (draggedColumnElement.current !== dropHeader) {
                ...
                const dragIndex = DomHandler.index(
                    draggedColumnElement.current,
                );
        ...
        if (draggedColumnElement.current) {
            ...
            if (allowDrop) {
                ...
                const isSameColumn = (col1, col2) =>
                    getColumnProp(col1, "columnKey") ||
                    getColumnProp(col2, "columnKey")
                        ? ObjectUtils.equals(
                              col1.props,
                              col2.props,
                              "columnKey",
                ...
                const columnOrder = columns.reduce((orders, col) => {
                    orders.push(
                        getColumnProp(col, "columnKey") ||
                            getColumnProp(col, "field"),
                    );
...
`;
    ...
    const onSortChange = (e) => {
        ...
        let sortField =
            getColumnProp(column, "sortField") ||
        ...
        if (props.sortMode === "multiple") {
            ...
            sortOrder = sortMeta
                ? getCalculatedSortOrder(sortMeta.order)
                : sortOrder;
        ...
        if (
            ...
            (groupRowsSortMetaState ||
                (multiSortMeta.length &&
                    props.groupRowsBy === multiSortMeta[0].field))
        ) {
        ...
        const removeEmptyFilters = ([key, value]) => {
            if (value.constraints) {
                ...
                if (filteredConstraints.length > 0) {
                    return [
                        key,
                        { ...value, constraints: filteredConstraints },
                    ];
                }
            ...
            for (const prop in activeFilters) {
                ...
                if (
                    ...
                    if (filterMeta.operator) {
                        for (
                            let j = 0;
                            j < filterMeta.constraints.length;
                            j++
                        ) {
                            ...
                            if (
                                (filterMeta.operator === FilterOperator.OR &&
                                    localMatch) ||
                                (filterMeta.operator === FilterOperator.AND &&
                                    !localMatch)
                            ) {
                                break;
                            }
    ...
    const processedData = (localState) => {
        ...
        if (!props.lazy) {
            if (data && data.length) {
                ...
                if (sortColumn) {
                    columnSortable.current = getColumnProp(
                        sortColumn,
                        "sortable",
                    );
    ...
    const createTableBody = (
        ...
        const spacerBody = ObjectUtils.isNotEmpty(spacerStyle) ? (
            <TableBody
                hostName="DataTable"
                style={{
                    height: `calc(${spacerStyle.height} - ${
        ...
        const wrapperProps = mergeProps(
            {
                ...
                style: {
                    ...
                    maxHeight: _isVirtualScrollerDisabled
                        ? props.scrollHeight
                        : null,
                },
            },
            ...
            <div ref={wrapperRef} {...wrapperProps}>
                <VirtualScroller
                    ...
                    style={{
                        ...
                        ...{
                            height:
                                props.scrollHeight !== "flex"
                                    ? props.scrollHeight
                                    : undefined,
                        },
                    }}
                    scrollHeight={
                        props.scrollHeight !== "flex" ? undefined : "100%"
                    }
