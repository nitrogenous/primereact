...
export const DataTable = React.forwardRef((inProps, ref) => {
	...
	const [multiSortMetaState, setMultiSortMetaState] = React.useState(
		props.multiSortMeta,
	);
	const [filtersState, setFiltersState] = React.useState(props.filters);
	const [columnOrderState, setColumnOrderState] = React.useState([]);
	const [groupRowsSortMetaState, setGroupRowsSortMetaState] =
		React.useState(null);
	...
	const [bindDocumentMouseMoveListener, unbindDocumentMouseMoveListener] =
		useEventListener({
			type: "mousemove",
			listener: (event) => {
	...
	const [bindDocumentMouseUpListener, unbindDocumentMouseUpListener] =
		useEventListener({
			type: "mouseup",
			listener: () => {
				if (columnResizing.current) {
	...
	const isVirtualScrollerDisabled = () => {
		return (
			ObjectUtils.isEmpty(props.virtualScrollerOptions) || !props.scrollable
		);
	};
	...
	const getColumns = (ignoreReorderable) => {
		...
		if (!ignoreReorderable && props.reorderableColumns && columnOrderState) {
			...
			return [
				...orderedColumns,
				...columns.filter((col) => orderedColumns.indexOf(col) < 0),
			];
		}
	...
	const restoreState = () => {
		...
		if (isCustomStateStorage()) {
			...
			const reviver = (key, value) => typeof value === "string" && dateFormat.test(value)
					? new Date(value)
					: value;
		...
		if (ObjectUtils.isNotEmpty(restoredState)) {
			if (props.paginator) {
				if (props.onPage) {
					...
					props.onPage(
						createEvent(
							getOnPageParams(restoredState.first, restoredState.rows),
						),
					);
				} else {
	...
	const addColumnWidthStyles = (widths) => {
		createStyleElement();
		let innerHTML = "";
		const selector = `[data-pc-name="datatable"][${
			attributeSelector.current
		}] > [data-pc-section="wrapper"] ${
			isVirtualScrollerDisabled() ? "" : '> [data-pc-name="virtualscroller"]'
		...
		widths.forEach((width, index) => {
			...
			innerHTML += `
                ${selector} > [data-pc-section="thead"] > tr > th:nth-child(${
									index + 1
								}),
                ${selector} > [data-pc-section="tbody"] > tr > td:nth-child(${
									index + 1
								}),
                ${selector} > [data-pc-section="tfoot"] > tr > td:nth-child(${
									index + 1
								}) {
                    ${style}
                }
	...
	const findColumnByKey = (columns, key) => {
		return ObjectUtils.isNotEmpty(columns)
			? columns.find(
					(col) =>
						getColumnProp(col, "columnKey") === key ||
						getColumnProp(col, "field") === key,
			  )
			: null;
	};
	...
	const onEditingMetaChange = (e) => {
		...
		if (editing) {
			!meta &&
				(meta = editingMeta[editingKey] = { data: { ...rowData }, fields: [] });
			meta["fields"].push(field);
		} else if (meta) {
			const fields = meta["fields"].filter((f) => f !== field);

			!fields.length
				? delete editingMeta[editingKey]
				: (meta["fields"] = fields);
		}
					...
					if (wrapperRef.current) {
						updateTableWidth(
							DomHandler.findSingle(
								wrapperRef.current,
								'[data-pc-name="virtualscroller"] > table > tbody',
							),
						);
					}
				}
	...
	const onColumnHeaderMouseDown = (e) => {
		...
		if (
			...
			getColumnProp(column, "reorderable") !== false &&
			!getColumnProp(column, "frozen")
		) {
			if (
	...
	const onColumnHeaderCheckboxChange = (e, processedData) => {
		if (props.onSelectAllChange) {
			...
			const data = props.selectionPageOnly
				...
				: processedData;
			let selection =
				props.selectionPageOnly && props.selection
					? props.selection.filter((s) => !data.some((d) => isEquals(s, d)))
					: [];

			if (checked) {
				selection = ObjectUtils.isNotEmpty(props.frozenValue)
					? [...selection, ...props.frozenValue, ...data]
					: [...selection, ...data];
	...
	const onColumnHeaderDragStart = (e) => {
		...
		colReorderIconWidth.current = DomHandler.getHiddenElementOuterWidth(
			reorderIndicatorUpRef.current,
		);
		colReorderIconHeight.current = DomHandler.getHiddenElementOuterHeight(
			reorderIndicatorUpRef.current,
		);
		...
		if (
			...
			if (draggedColumnElement.current !== dropHeader) {
				...
				if (event.pageX > columnCenter && dragIndex < dropIndex) {
					...
					reorderIndicatorUpRef.current.style.left =
						targetLeft - Math.ceil(colReorderIconWidth.current / 2) + "px";
					reorderIndicatorDownRef.current.style.left =
						targetLeft - Math.ceil(colReorderIconWidth.current / 2) + "px";
					dropPosition.current = -1;
				}
	...
	const onColumnHeaderDrop = (e) => {
		...
		if (draggedColumnElement.current) {
			...
			if (
				allowDrop &&
				((dropIndex - dragIndex === 1 && dropPosition.current === -1) ||
					(dragIndex - dropIndex === 1 && dropPosition.current === 1))
			) {
				allowDrop = false;
			}
			...
			if (allowDrop) {
				...
				const isSameColumn = (col1, col2) =>
					getColumnProp(col1, "columnKey") || getColumnProp(col2, "columnKey")
						? ObjectUtils.equals(col1.props, col2.props, "columnKey")
						: ObjectUtils.equals(col1.props, col2.props, "field");
				const dragColIndex = columns.findIndex((child) =>
					isSameColumn(child, draggedColumn.current),
				);
				let dropColIndex = columns.findIndex((child) =>
				...
				const remainingItems = widths.filter(
					(items, index) => index !== dragColIndex,
				);
				...
				const columnOrder = columns.reduce((orders, col) => {
					orders.push(
						getColumnProp(col, "columnKey") || getColumnProp(col, "field"),
					);
...
[data-pc-name="datatable"][${attributeSelector.current}] {
	...
	const createResponsiveStyle = () => {
		if (!responsiveStyleElement.current) {
			...
			const tableSelector = `.p-datatable-wrapper ${
				isVirtualScrollerDisabled() ? "" : "> .p-virtualscroller"
			} > .p-datatable-table`;
...
`;
	...
	const destroyResponsiveStyle = () => {
		responsiveStyleElement.current = DomHandler.removeInlineStyle(
			responsiveStyleElement.current,
		);
	};
	...
	const destroyBeforeResizeStyleElement = () => {
		beforeResizeStyleElement.current = DomHandler.removeInlineStyle(
			beforeResizeStyleElement.current,
		);
	};
	...
	const onSortChange = (e) => {
		...
		let sortField =
			getColumnProp(column, "sortField") || getColumnProp(column, "field");
		...
		if (props.sortMode === "multiple") {
			...
			if (sortOrder) {
				multiSortMeta = metaKey
					? multiSortMeta
					: multiSortMeta.filter((meta) =>
							sortableDisabledFields.some((field) => field === meta.field),
					  );
			...
			sortOrder =
				getSortField() === sortField
					? getCalculatedSortOrder(getSortOrder())
					: sortOrder;
	...
	const addSortMeta = (meta, multiSortMeta) => {
		const index = multiSortMeta.findIndex(
			(sortMeta) => sortMeta.field === meta.field,
		);
	...
	const sortMultiple = (data, multiSortMeta = []) => {
		if (
			props.groupRowsBy &&
			(groupRowsSortMetaState ||
				(multiSortMeta.length && props.groupRowsBy === multiSortMeta[0].field))
		) {
		...
		if (columnSortable.current && columnSortFunction.current) {
			const meta = multiSortMeta.find(
				(meta) => meta.field === columnField.current,
			);
	...
	const multisortField = (data1, data2, multiSortMeta, index, comparator) => {
		...
		if (ObjectUtils.compare(value1, value2, comparator) === 0) {
			return multiSortMeta.length - 1 > index
				? multisortField(data1, data2, multiSortMeta, index + 1, comparator)
				: 0;
		}

		return compareValuesOnSort(
			value1,
			value2,
			comparator,
	...
	const filterLocal = (data, filters) => {
		...
		if (isGlobalFilter) {
			globalFilterFieldsArray =
				...
				columns
					.filter((col) => !getColumnProp(col, "excludeGlobalFilter"))
					.map(
						(col) =>
							getColumnProp(col, "filterField") || getColumnProp(col, "field"),
					);
		}
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (
				...
				for (let j = 0; j < globalFilterFieldsArray.length; j++) {
					...
					const matchMode = activeFilters["global"]
						? activeFilters["global"].matchMode
						: props.globalFilterMatchMode;
					const value = activeFilters["global"]
						? activeFilters["global"].value
						: props.globalFilter;

					globalMatch = FilterService.filters[matchMode](
		...
		if (
			filteredValue.length === props.value.length ||
	...
	const executeLocalFilter = (field, rowData, filterMeta, index) => {
		...
		return (
			ObjectUtils.isFunction(filterConstraint) &&
			filterConstraint(dataFieldValue, filterValue, props.filterLocale, index)
		);
	};
			...
			cloned = columns.reduce((filters, col) => {
				...
				const matchMode =
					...
					PrimeReact.filterMatchModeOptions[dataType]
						? (context && context.filterMatchModeOptions[dataType][0]) ||
				...
				if (filterFunction) {
					FilterService.register(`custom_${field}`, (...args) =>
						filterFunction(...args, { column: col }),
					);
				}

				filters[field] =
					props.filterDisplay === "menu"
						? { operator: FilterOperator.AND, constraints: [constraint] }
						: constraint;
	...
	const filter = (value, field, matchMode, index = 0) => {
		...
		constraint = meta
			? { value, matchMode: matchMode || constraint.matchMode }
			: { value, matchMode };
		props.filterDisplay === "menu" && meta && meta.operator
			? (filters[field].constraints[index] = constraint)
	...
	const resetScroll = () => {
		if (wrapperRef.current) {
			const scrollableContainer = !isVirtualScrollerDisabled()
				? DomHandler.findSingle(
						wrapperRef.current,
	...
	const exportCSV = (options) => {
		...
		columns.forEach((column, i) => {
			const [field, header, exportHeader, exportable] = [
		...
		data.forEach((record) => {
			csv += "\n";
			columns.forEach((column, i) => {
				const [colField, exportField, exportable] = [
					getColumnProp(column, "field"),
	...
	const closeEditingRows = () => {
		DomHandler.find(
			document.body,
			'[data-pc-section="roweditorcancelbuttonprops"]',
	...
	const processedData = (localState) => {
		...
		if (!props.lazy) {
			if (data && data.length) {
				...
				const sortField =
					(localState && localState.sortField) || getSortField();
				const sortOrder =
					(localState && localState.sortOrder) || getSortOrder();
				const multiSortMeta =
					(localState && localState.multiSortMeta) || getMultiSortMeta();
				const columns = getColumns();
				...
				if (sortColumn) {
					...
					columnSortFunction.current = getColumnProp(
						sortColumn,
						"sortFunction",
					);
				}
	...
	const createLoader = () => {
		if (props.loading) {
			...
			const icon = props.loadingIcon || (
				<SpinnerIcon {...loadingIconProps} spin />
			);
	...
	const createTableHeader = (options, empty, _isVirtualScrollerDisabled) => {
		...
		const data =
			_isVirtualScrollerDisabled || virtualScrollerProps.lazy
				? processedData
				: virtualScrollerProps.items;
			...
			<div ref={wrapperRef} {...wrapperProps}>
				<VirtualScroller
					...
					style={{
						...
						...{
							height:
								props.scrollHeight !== "flex" ? props.scrollHeight : undefined,
						},
					}}
		...
		return (
			<Paginator
				...
				className={classNames(
					...
					ptCallbacks.cx("paginator", { position }),
				)}
	...
	const createReorderIndicators = () => {
		if (props.reorderableColumns) {
			...
			const reorderIndicatorUpIconProps = mergeProps(
				ptCallbacks.ptm("reorderIndicatorUpIcon"),
			);
			const reorderIndicatorUpIcon = IconUtils.getJSXIcon(
				props.reorderIndicatorUpIcon || (
					<ArrowDownIcon {...reorderIndicatorUpIconProps} />
				),
			...
			const reorderIndicatorDownIconProps = mergeProps(
				ptCallbacks.ptm("reorderIndicatorDownIcon"),
			);
			const reorderIndicatorDownIcon = IconUtils.getJSXIcon(
				props.reorderIndicatorDownIcon || (
					<ArrowUpIcon {...reorderIndicatorDownIconProps} />
				),
	...
	const rootProps = mergeProps(
		{
			...
			className: classNames(
				props.className,
				ptCallbacks.cx("root", { selectable }),
			),
			style: props.style,
