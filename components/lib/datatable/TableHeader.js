...
export const TableHeader = React.memo((props) => {
    const [sortableDisabledFieldsState, setSortableDisabledFieldsState] =
        React.useState([]);
    const [allSortableDisabledState, setAllSortableDisabledState] =
    ...
    const getColumnGroupProps = () => {
        return props.headerColumnGroup
            ? ptmo(ColumnGroupBase.getCProps(props.headerColumnGroup))
            : undefined;
    };
    ...
    const getColumnGroupPTOptions = (key) => {
        ...
        return mergeProps(
            ptm(`columnGroup.${key}`, { columnGroup: columnGroupMetaData }),
            ptm(`columnGroup.${key}`, columnGroupMetaData),
            ptmo(cGProps, key, columnGroupMetaData),
        );
    };
    ...
    const getRowPTOptions = (row, key) => {
        ...
        return mergeProps(
            ptm(`row.${key}`, { row: rowMetaData }),
            ptm(`row.${key}`, rowMetaData),
            ptmo(rProps, key, rowMetaData),
        );
    };

    const isColumnSorted = (column) => {
        return props.sortField !== null
            ? getColumnProp(column, "field") === props.sortField ||
                  getColumnProp(column, "sortField") === props.sortField
            : false;
    };
                ...
                const headerCellProps = mergeProps(
                    {
                        ...
                        className: classNames(
                            filterHeaderClassName,
                            className,
                            cx("headerCell", { frozen, column: col }),
                        ),
                        key: colKey,
                    },
    ...
    const createContent = () => {
        if (props.headerColumnGroup) {
            const rows = React.Children.toArray(
                ColumnGroupBase.getCProp(props.headerColumnGroup, "children"),
            );
