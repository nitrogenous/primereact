...
export const ColumnFilter = React.memo((props) => {
    ...
    const isTargetClicked = (target) => {
        return (
            iconRef.current &&
            (iconRef.current.isSameNode(target) ||
    ...
    const addConstraint = () => {
        const filterConstraintAddCallback = getColumnProp(
    ...
    const findNextItem = (item) => {
        ...
        return nextItem
            ? DomHandler.getAttribute(
                  nextItem,
                  "data-p-column-filter-separator",
              ) === true
                        ...
                        const filterRowItemProps = mergeProps(
                            {
                                ...
                                onKeyDown: (e) =>
                                    onRowMatchModeKeyDown(e, matchMode),
                                tabIndex,
                            },
                            getColumnPTOptions("filterRowItem", {
                                context: {
        ...
        return (
            <div {...filterConstraintsProps}>
                {_fieldConstraints.map((fieldConstraint, i) => {
                    const matchModeDropdown = createMatchModeDropdown(
                        fieldConstraint,
                        i,
                    );
                    const menuFilterElement = createMenuFilterElement(
                        fieldConstraint,
                        i,
                    );
    ...
    const createOverlay = () => {
        ...
        const items =
            props.display === "row" ? createRowItems() : createItems();
