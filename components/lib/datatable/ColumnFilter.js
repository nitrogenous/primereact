...
export const ColumnFilter = React.memo((props) => {
	...
	const hasFilter = () => {
		...
		return filterStoreModel.operator
			? !isFilterBlank(filterModel.constraints[0].value)
			: !isFilterBlank(filterModel.value);
	};
	...
	const isShowMenuButton = () => {
		return (
			getColumnProp("showFilterMenu") &&
			(props.display === "row" ? getColumnProp("dataType") !== "boolean" : true)
		);
	};
	...
	const isShowMatchModes = () => {
		return (
			getColumnProp("dataType") !== "boolean" &&
			getColumnProp("showFilterMatchModes") &&
			matchModes() &&
			getColumnProp("showFilterMenuOptions")
		);
	};

	const isShowOperator = () => {
		return (
			getColumnProp("showFilterOperator") &&
			filterModel &&
	...
	const isShowAddConstraint = () => {
		return (
			getColumnProp("showAddButton") &&
			filterModel &&
			filterModel.operator &&
			fieldConstraints() &&
			fieldConstraints().length < getColumnProp("maxConstraints") &&
			getColumnProp("showFilterMenuOptions")
		);
	};
	...
	const isTargetClicked = (target) => {
		return (
			iconRef.current &&
			(iconRef.current.isSameNode(target) || iconRef.current.contains(target))
		);
	};
	...
	const findDataType = () => {
		...
		const hasMatchMode = (key) =>
			...
			PrimeReact.filterMatchModeOptions[key].some((mode) => mode === matchMode);
		...
		if (matchMode === "custom" && !hasMatchMode(dataType)) {
			...
			return (
				Object.keys(
					(context && context.filterMatchModeOptions) ||
	...
	const applyFilter = () => {
		...
		filterApplyClickCallback &&
			filterApplyClickCallback({ field, constraints: filterModel });
		props.onFilterApply();
		hide();
	};
		...
		switch (event.key) {
			...
			case "ArrowDown":
				if (overlayVisibleState) {
					const focusable = DomHandler.getFirstFocusableElement(
						overlayRef.current,
					);
	...
	const onInputChange = (event, index) => {
		...
		if (
			props.display === "menu" &&
			ObjectUtils.isNotEmpty(filterField.constraints)
		) {
	...
	const onRowMatchModeChange = (matchMode) => {
		const filterMatchModeChangeCallback = getColumnProp(
		...
		filterMatchModeChangeCallback &&
			filterMatchModeChangeCallback({ field, matchMode });
		props.onFilterChange(filters);
		props.onFilterApply();
		hide();
	};
	...
	const onOperatorChange = (e) => {
		const filterOperationChangeCallback = getColumnProp(
		...
		filterOperationChangeCallback &&
			filterOperationChangeCallback({ field, operator: value });
	...
	const onMenuMatchModeChange = (value, index) => {
		...
		props.onFilterChange(filters);
		filterMatchModeChangeCallback &&
			filterMatchModeChangeCallback({ field, matchMode: value, index: index });
	...
	const addConstraint = () => {
		...
		const defaultConstraint = getDefaultConstraint();
		const filters = { ...props.filters };
		const newConstraint = { value: null, matchMode: defaultConstraint.matchMode };

		filters[field].constraints.push(newConstraint);
		filterConstraintAddCallback &&
			filterConstraintAddCallback({ field, constraint: newConstraint });
		props.onFilterChange(filters);
	...
	const removeConstraint = (index) => {
		const filterConstraintRemoveCallback = getColumnProp(
		...
		filterConstraintRemoveCallback &&
			filterConstraintRemoveCallback({ field, constraint: removedConstraint });
		props.onFilterChange(filters);
	...
	const findNextItem = (item) => {
		...
		return nextItem
			? DomHandler.getAttribute(nextItem, "data-p-column-filter-separator") ===
			  true
	...
	const findPrevItem = (item) => {
		...
		return prevItem
			? DomHandler.getAttribute(prevItem, "data-p-column-filter-separator") ===
			  true
	...
	const onOverlayEnter = () => {
		ZIndexUtils.set(
			...
			(context && context.zIndex["overlay"]) || PrimeReact.zIndex["overlay"],
		);
		...
		DomHandler.alignOverlay(
			overlayRef.current,
			iconRef.current,
			(context && context.appendTo) || PrimeReact.appendTo,
			false,
		);
	...
	const filterCallback = (value, index = 0) => {
		...
		props.display === "menu" && meta && meta.operator
	...
	const createFilterElement = (model, index) => {
		...
		return getColumnProp("filterElement") ? (
			ObjectUtils.getJSXElement(getColumnProp("filterElement"), {
				field,
				index,
				filterModel: model,
				value,
	...
	const createMenuFilterElement = (fieldConstraint, index) => {
		return props.display === "menu"
			? createFilterElement(fieldConstraint, index)
			: null;
	};
	...
	const createMenuButton = () => {
		...
		const columnFilterIcon = IconUtils.getJSXIcon(
			icon,
			{ ...filterIconProps },
			{ props },
		);
	...
	const createClearButton = () => {
		...
		const icon = props.filterClearIcon || (
			<FilterSlashIcon {...filterClearIconProps} />
		);
		const filterClearIcon = IconUtils.getJSXIcon(
			icon,
		...
		return (
			<div {...filterConstraintsProps}>
				{_fieldConstraints.map((fieldConstraint, i) => {
					...
					const filterRemoveProps = mergeProps(
						getColumnPTOptions("filterRemove"),
					);
	...
	const createOverlay = () => {
		...
		const filterHeader = ObjectUtils.getJSXElement(
			getColumnProp("filterHeader"),
			{ field, filterModel, filterApplyCallback },
		);
		const filterFooter = ObjectUtils.getJSXElement(
			getColumnProp("filterFooter"),
			{ field, filterModel, filterApplyCallback },
		);
