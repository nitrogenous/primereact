...
export const BodyRow = React.memo((props) => {
    ...
    const getBodyRowPTOptions = (key) => {
        return ptm(key, {
            ...
            context: {
                ...
                selected:
                    (!props.allowCellSelection && props.selected) ||
                    props.contextMenuSelected,
    ...
    const isGrouped = (column) => {
        ...
        if (props.groupRowsBy && columnField) {
            return Array.isArray(props.groupRowsBy)
                ? props.groupRowsBy.indexOf(columnField) > -1
                : props.groupRowsBy === columnField;
        }
    ...
    const getTabIndex = () => {
        return isFocusable() && !props.allowCellSelection
            ? props.rowIndex === 0
                ? props.tabIndex
                : -1
            : null;
    ...
    const findNextSelectableRow = (row) => {
        ...
        return nextRow
            ? DomHandler.getAttribute(nextRow, "data-p-selectable-row") === true
                ? nextRow
                : findNextSelectableRow(nextRow)
            : null;
    };
    ...
    const findPrevSelectableRow = (row) => {
        ...
        return prevRow
            ? DomHandler.getAttribute(prevRow, "data-p-selectable-row") === true
                ? prevRow
                : findPrevSelectableRow(prevRow)
            : null;
    };
        ...
        if (props.onRowEditChange) {
            ...
            if (dataKey) {
                const dataKeyValue = String(
                    ObjectUtils.resolveFieldData(data, dataKey),
                );
                ...
                if (!isEditing) {
                    ...
                    const newDataKeyValue = String(
                        ObjectUtils.resolveFieldData(newData, dataKey),
                    );
                ...
                if (editingRowIndex !== -1)
                    editingRows = editingRows.filter(
                        (val, i) => i !== editingRowIndex,
                    );
    ...
    const onEditSave = (e) => {
        ...
        const valid = props.rowEditValidator
            ? props.rowEditValidator(newData, { props: props.tableProps })
            : true;
    ...
    const createContent = () => {
        return props.columns.map((col, i) => {
            if (shouldRenderBodyCell(props.value, col, props.index)) {
                const key = `${props.rowIndex}_${
                    getColumnProp(col, "columnKey") ||
                    getColumnProp(col, "field")
                }_${i}`;
                const rowSpan =
                    props.rowGroupMode === "rowspan"
                        ? calculateRowGroupSize(props.value, col, props.index)
                        : null;
    ...
    const rowClassName = ObjectUtils.getPropValue(
        props.rowClassName,
        props.rowData,
        { props: props.tableProps },
    );
    const style = {
        height: props.virtualScrollerOptions
            ? props.virtualScrollerOptions.itemSize
            : undefined,
    };
