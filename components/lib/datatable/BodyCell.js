...
export const BodyCell = React.memo((props) => {
	...
	const [editingRowDataState, setEditingRowDataState] = React.useState(
		props.rowData,
	);
	...
	const editingKey = props.dataKey
		? (props.rowData && props.rowData[props.dataKey]) || props.rowIndex
		: props.rowIndex;
	...
	const equalsData = (data) => {
		return props.compareSelectionBy === "equals"
			? data === props.rowData
			: ObjectUtils.equals(data, props.rowData, props.dataKey);
	};

	const equals = (selectedCell) => {
		return (
			selectedCell &&
			(selectedCell.rowIndex === props.rowIndex ||
				equalsData(selectedCell.rowData)) &&
			(selectedCell.field === field || selectedCell.cellIndex === props.index)
		);
	};
	...
	const isOutsideClicked = (target) => {
		return (
			elementRef.current &&
			!(
				elementRef.current.isSameNode(target) ||
	...
	const getVirtualScrollerOption = (option) => {
		return props.virtualScrollerOptions
			? props.virtualScrollerOptions[option]
			: null;
	};
	...
	const getStyle = () => {
		...
		return getColumnProp("frozen")
			? Object.assign({}, columnStyle, bodyStyle, styleObjectState)
			: Object.assign({}, columnStyle, bodyStyle);
	};
	...
	const getEditingRowData = () => {
		return props.editingMeta && props.editingMeta[editingKey]
			? props.editingMeta[editingKey].data
			: props.rowData;
	};

	const getTabIndex = (cellSelected) => {
		return props.allowCellSelection
			? cellSelected
				? 0
				: props.rowIndex === 0 && props.index === 0
				  ? props.tabIndex
				  : -1
			: null;
	};
	...
	const findNextSelectableCell = (cell) => {
		...
		return nextCell
			? DomHandler.getAttribute(nextCell, "data-p-selectable-cell")
				? nextCell
				: findNextSelectableCell(nextCell)
			: null;
	};
	...
	const findPrevSelectableCell = (cell) => {
		...
		return prevCell
			? DomHandler.getAttribute(prevCell, "data-p-selectable-cell")
				? prevCell
				: findPrevSelectableCell(prevCell)
			: null;
	};
	...
	const findDownSelectableCell = (cell) => {
		...
		return downRow && downCell
			? DomHandler.getAttribute(downRow, "data-p-selectable-row") &&
			  DomHandler.getAttribute(downCell, "data-p-selectable-cell")
				? downCell
				: findDownSelectableCell(downCell)
			: null;
	};
	...
	const findUpSelectableCell = (cell) => {
		...
		return upRow && upCell
			? DomHandler.getAttribute(upRow, "data-p-selectable-row") &&
			  DomHandler.getAttribute(upCell, "data-p-selectable-cell")
				? upCell
				: findUpSelectableCell(upCell)
			: null;
	};
	...
	const focusOnElement = () => {
		...
		tabindexTimeout.current = setTimeout(() => {
				...
				const focusableEl =
					props.editMode === "cell"
						? DomHandler.getFirstFocusableElement(
								elementRef.current,
								':not([data-pc-section="editorkeyhelperlabel"])',
						  )
			...
			keyHelperRef.current &&
				(keyHelperRef.current.tabIndex = editingState ? -1 : 0);
		}, 1);
	};
	...
	const editorCallback = (val) => {
		const editingRowData = { ...editingRowDataState };
	...
	const onClick = (event) => {
		...
		if (
			...
			isEditable() &&
			!editingState &&
			(props.selectOnEdit || (!props.selectOnEdit && props.selected))
		) {
	...
	const onBlur = (event) => {
		...
		if (
			props.editMode !== "row" &&
			editingState &&
			getColumnProp("cellEditValidatorEvent") === "blur"
		) {
			switchCellToViewMode(event, true);
		}
	};
	...
	const createLoading = () => {
		const options = getVirtualScrollerOption("getLoaderOptions")(
			props.rowIndex,
			{
				cellIndex: props.index,
				cellFirst: props.index === 0,
				cellLast:
					props.index === getVirtualScrollerOption("columns").length - 1,
		...
		const content = ObjectUtils.getJSXElement(
			getVirtualScrollerOption("loadingTemplate"),
			options,
		);
	...
	const createElement = () => {
		...
		const rowEditor = ObjectUtils.getPropValue(
			getColumnProp("rowEditor"),
			props.rowData,
			columnBodyOptions,
		);
		const expander = ObjectUtils.getPropValue(
			getColumnProp("expander"),
			props.rowData,
			columnBodyOptions,
		);
		const cellClassName = ObjectUtils.getPropValue(
			props.cellClassName,
			value,
			columnBodyOptions,
		);
		const bodyClassName = ObjectUtils.getPropValue(
			getColumnProp("bodyClassName"),
			props.rowData,
			columnBodyOptions,
		);
		...
		const title = props.responsiveLayout === "stack" && (
			<span {...columnTitleProps}>
				{ObjectUtils.getJSXElement(header, { props: props.tableProps })}
			</span>
		);
		...
		if (selectionMode) {
			...
			if (showSelection) {
				const ariaLabelField =
					props.selectionAriaLabel || props.tableProps.dataKey;
				const ariaLabelText = ObjectUtils.resolveFieldData(
					props.rowData,
					ariaLabelField,
				);

				label = `${
					props.selected ? ariaLabel("unselectLabel") : ariaLabel("selectLabel")
				} ${ariaLabelText}`;
			}
			...
			const icon = props.expanded
				? props.expandedRowIcon || <ChevronDownIcon {...rowTogglerIconProps} />
				: props.collapsedRowIcon || (
			...
			const label = `${
				props.expanded ? ariaLabel("collapseLabel") : ariaLabel("expandLabel")
			} ${ariaLabelText}`;
			const expanderProps = {
				onClick: onRowToggle,
			...
			const rowEditorCancelIcon = IconUtils.getJSXIcon(
				props.rowEditorCancelIcon || (
					<TimesIcon {...rowEditorCancelIconProps} />
				),
		...
		if (!isRowEditor && editor) {
			...
			const editorKeyHelperLabelProps = mergeProps(
				getColumnPTOptions("editorKeyHelper"),
			);
			/* eslint-disable */
			editorKeyHelper = (
		...
		const bodyCellProps = mergeProps(
			{
				...
				"data-p-selectable-cell":
					...
					props.isSelectable({ data: getCellParams(), index: props.rowIndex }),
	...
	return getVirtualScrollerOption("loading")
		? createLoading()
		: createElement();
});
