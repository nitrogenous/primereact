...
export const HeaderCell = React.memo((props) => {
	...
	const isSortableDisabled = () => {
		return (
			...
			(getColumnProp("sortable") &&
				(props.allSortableDisabled || getColumnProp("sortableDisabled")))
		);
	};
	...
	const onKeyDown = (event) => {
		if (
			...
			DomHandler.getAttribute(event.currentTarget, "data-p-sortable-column") ===
				"true"
		) {
			onClick(event);
	...
	const createBadge = ({ metaIndex }) => {
		if (metaIndex !== -1 && isBadgeVisible()) {
			const value =
				props.groupRowsBy && props.groupRowsBy === props.groupRowSortField
					? metaIndex
					: metaIndex + 1;
	...
	const createElement = () => {
		...
		const tabIndex =
			getColumnProp("sortable") && !_isSortableDisabled ? props.tabIndex : null;
		...
		const headerCellProps = mergeProps(
			{
				...
				"data-p-filter-column":
					!props.metaData.props.headerColumnGroup &&
					props.filterDisplay === "row",
