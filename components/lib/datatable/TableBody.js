...
export const TableBody = React.memo(
    React.forwardRef((props, ref) => {
        ...
        const [
            rowGroupHeaderStyleObjectState,
        ...
        const isSelectable = (options) => {
            return props.isDataSelectable
                ? props.isDataSelectable(options)
                : true;
        };
        ...
        const allowRowDrag = (event) => {
            return (
                (!allowCellSelection() && allowDrag(event)) ||
        ...
        const findIndex = (collection, rowData) => {
            return (collection || []).findIndex((data) =>
                equals(rowData, data),
            );
        };
        ...
        const onMultipleSelection = ({
            ...
            if (selected) {
                ...
                selection = ObjectUtils.isObject(selection)
                    ? [selection]
                    : selection;
                selection =
                    toggleable && isMultipleSelection()
                        ? [...selection, data]
                        : [data];
                onSelect({ originalEvent, data, type });
            }
        ...
        const onRowToggle = (event) => {
            ...
            if (hasDataKey) {
                ...
                const expandedRowIndex = findIndex(
                    props.expandedRows,
                    event.data,
                );
        ...
        const onRowDragOver = (e) => {
            ...
            if (rowDragging.current && draggedRowIndex.current !== index) {
                ...
                const rowMidY =
                    rowY + DomHandler.getOuterHeight(rowElement) / 2;
        ...
        const createRow = (rowData, rowIndex, index, expanded) => {
            if (!props.expandableRowGroups || expanded) {
                const selected = isSelectionEnabled()
                    ? isSelected(rowData)
                    : false;
        ...
        const createExpansion = (rowData, rowIndex, expanded, colSpan) => {
            if (
                expanded &&
                !(isSubheaderGrouping && props.expandableRowGroups)
            ) {
