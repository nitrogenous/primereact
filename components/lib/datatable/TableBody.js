...
export const TableBody = React.memo(
	React.forwardRef((props, ref) => {
		...
		const [rowGroupHeaderStyleObjectState, setRowGroupHeaderStyleObjectState] =
			React.useState({});
		...
		const isSubheaderGrouping =
			props.rowGroupMode && props.rowGroupMode === "subheader";
		...
		const isRadioSelectionModeInColumn =
			props.selectionModeInColumn === "single";
		const isCheckboxSelectionModeInColumn =
			props.selectionModeInColumn === "multiple";

		const equals = (data1, data2) => {
			if (allowCellSelection())
				return (
					(data1.rowIndex === data2.rowIndex ||
						data1.rowData === data2.rowData) &&
					(data1.field === data2.field || data1.cellIndex === data2.cellIndex)
				);
			else
		...
		const isMultipleSelection = () => {
			return (
				(props.selectionMode === "multiple" && !isRadioSelectionModeInColumn) ||
				isCheckboxSelectionModeInColumn
			);
		};
						...
						if (props.expandedRows) {
							if (Array.isArray(props.expandedRows)) {
								expanded = props.expandedRows.some(
									(row) =>
										ObjectUtils.resolveFieldData(row, props.dataKey) === rowId,
								);
							} else {
		...
		const isRowGroupExpanded = (rowData) => {
			if (props.dataKey === props.groupRowsBy)
				return Object.keys(props.expandedRows).some((data) =>
					ObjectUtils.equals(
						data,
		...
		const allowRowDrag = (event) => {
			return (
				(!allowCellSelection() && allowDrag(event)) || props.reorderableRows
			);
		};
		...
		const allowRowSelection = () => {
			return (
				(props.selectionMode || props.selectionModeInColumn) &&
		...
		const shouldRenderRowGroupHeader = (value, rowData, i) => {
			...
			if (prevRowData) {
				const previousRowFieldData = ObjectUtils.resolveFieldData(
					prevRowData,
					props.groupRowsBy,
				);
			...
			if (selected) {
				if (toggleable) {
					...
					props.selection.forEach((d) =>
						onUnselect({ originalEvent, data: d, type }),
					);
					selection = [data];
					onSelect({ originalEvent, data, type });
				}
				...
				selection =
					toggleable && isMultipleSelection() ? [...selection, data] : [data];
				onSelect({ originalEvent, data, type });
			}
		...
		const onRangeSelection = (event, type) => {
			...
			rangeRowIndex.current = allowCellSelection()
				? event.rowIndex
				: event.index;
		...
		const selectRange = (event) => {
			...
			return allowCellSelection()
				? selectRangeOnCell(event, rangeStart, rangeEnd)
				: selectRangeOnRow(event, rangeStart, rangeEnd);
		};
		...
		const enableDragSelection = (event) => {
			if (props.dragSelection && !dragSelectionHelper.current) {
				...
				dragSelectionHelper.current.setAttribute(
					"p-datatable-drag-selection-helper",
					"true",
				);
				!isUnsyled &&
					DomHandler.addClass(
						dragSelectionHelper.current,
						"p-datatable-drag-selection-helper",
					);
		...
		const focusOnElement = (event, isFocused) => {
			...
			if (!allowCellSelection() && props.selectionAutoFocus) {
				if (isCheckboxSelectionModeInColumn) {
					const checkbox = DomHandler.findSingle(
						target,
		...
		const changeTabIndex = (event, type) => {
			...
			const isSelectable =
				DomHandler.getAttribute(
					target,
					type === "cell" ? "data-p-selectable-cell" : "data-p-selectable-row",
				) === true;

			if (isSelectable) {
				const selector = type === "cell" ? "tr > td" : "tr";
				const tabbableEl = DomHandler.findSingle(
					elementRef.current,
					`${selector}[tabindex="${props.tabIndex}"]`,
				);
		...
		const onRowToggle = (event) => {
			...
			const hasDataKey = props.groupRowsBy
				? dataKey === props.groupRowsBy
				: !!dataKey;

			if (hasDataKey) {
				const dataKeyValue = String(
		...
		const onRowDrop = (e) => {
			...
			if (droppedRowIndex.current != null) {
				const dropIndex =
					draggedRowIndex.current > droppedRowIndex.current
						? droppedRowIndex.current
						: droppedRowIndex.current === 0
						  ? 0
		...
		const onCellMouseUp = (event) => {
			const isSameCell =
				event.rowIndex === anchorRowIndex.current &&
				event.cellIndex === anchorCellIndex.current;
		...
		const createGroupHeader = (rowData, rowIndex, expanded, colSpan) => {
			if (
				isSubheaderGrouping &&
				shouldRenderRowGroupHeader(props.value, rowData, rowIndex - props.first)
			) {
				...
				let content = ObjectUtils.getJSXElement(
					props.rowGroupHeaderTemplate,
					rowData,
					options,
				);
		...
		const createGroupFooter = (rowData, rowIndex, expanded, colSpan) => {
			if (
				...
				shouldRenderRowGroupFooter(
					props.value,
					rowData,
					rowIndex - props.first,
					expanded,
				)
			) {
				const content = ObjectUtils.getJSXElement(
					props.rowGroupFooterTemplate,
					rowData,
					{ index: rowIndex, colSpan, props: props.tableProps },
				);
		...
		const ptKey =
			props.className === "p-datatable-virtualscroller-spacer"
