...
export const FileUpload = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const chooseButtonLabel =
			props.chooseLabel || props.chooseOptions.label || localeOption("choose");
		const uploadButtonLabel =
			props.uploadLabel || props.uploadOptions.label || localeOption("upload");
		const cancelButtonLabel =
			props.cancelLabel || props.cancelOptions.label || localeOption("cancel");
		...
		const onFileSelect = (event) => {
			...
			if (
				...
				props.onBeforeSelect({ originalEvent: event, files: filesState }) ===
					false
			) {
				return;
			}
		...
		const isFileSelected = (file) => {
			return filesState.some(
				(f) => f.name + f.type + f.size === file.name + file.type + file.size,
			);
		};
			...
			if (props.customUpload) {
				...
				xhr.onreadystatechange = () => {
					if (xhr.readyState === 4) {
						...
						clear();
						setUploadedFilesState((prevUploadedFiles) => [
							...prevUploadedFiles,
							...files,
						]);
					}
		...
		const onDrop = (event) => {
			...
			const allowDrop =
				props.multiple ||
				(ObjectUtils.isEmpty(filesState) && files && files.length === 1);
		...
		const createChooseButton = () => {
			...
			const label = iconOnly ? (
				<span
					{...chooseButtonLabelProps}
					dangerouslySetInnerHTML={{ __html: "&nbsp;" }}
				/>
			) : (
				<span {...chooseButtonLabelProps}>{chooseButtonLabel}</span>
			);
			...
			const chooseIcon = IconUtils.getJSXIcon(
				icon,
				{ ...chooseIconProps },
				{ props },
			);
			const chooseButtonProps = mergeProps(
				{
					className: classNames(
						className,
						cx("chooseButton", { iconOnly, disabled, className, focusedState }),
					),
					style,
					onClick: choose,
		...
		const createFile = (file, index, badgeOptions) => {
			...
			const preview = isImage(file) ? (
				<img {...thumbnailProps} alt={file.name} />
			) : null;
		...
		const createFiles = () => {
			...
			const content = filesState.map((file, index) =>
		...
		const createEmptyContent = () => {
			return props.emptyTemplate && !hasFiles && !hasUploadedFiles
		...
		const createProgressBarContent = () => {
			if (props.progressBarTemplate) {
				...
				return ObjectUtils.getJSXElement(
					...
					defaultProgressBarTemplateOptions,
				);
			}
		...
		const createAdvanced = () => {
			...
			if (!props.auto) {
				...
				const uploadIcon = IconUtils.getJSXIcon(
					uploadOptions.icon || <UploadIcon {...uploadIconProps} />,
					{ ...uploadIconProps },
					{ props },
				);
				...
				const cancelIcon = IconUtils.getJSXIcon(
					cancelOptions.icon || <TimesIcon {...cancelIconProps} />,
					{ ...cancelIconProps },
					{ props },
				);
		...
		const createBasic = () => {
			...
			const chooseLabel = chooseOptions.iconOnly ? (
				...
				<span {...labelProps}>{chooseButtonLabel}</span>
			);
			...
			const icon = chooseOptions.icon ? (
				chooseOptions.icon
			) : !chooseOptions.icon && (!hasFiles || props.auto) ? (
				<PlusIcon {...chooseIconProps} />
			) : (
				!chooseOptions.icon &&
				hasFiles &&
				!props.auto && <UploadIcon {...chooseIconProps} />
			);
			const chooseIcon = IconUtils.getJSXIcon(
				icon,
				{ ...chooseIconProps },
			...
			const basicButtonProps = mergeProps(
				{
					className: classNames(
						...
						cx("basicButton", { hasFiles, disabled, focusedState }),
					),
