...
export const PanelMenu = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const onHeaderArrowDownKey = (event) => {
			const rootList =
				DomHandler.getAttribute(event.currentTarget, "data-p-highlight") ===
				true
		...
		const onHeaderArrowUpKey = (event) => {
			const prevHeader =
				findPrevHeader(event.currentTarget.parentElement) || findLastHeader();
			const rootList =
				DomHandler.getAttribute(prevHeader, "data-p-highlight") === true
					? DomHandler.findSingle(
							prevHeader.nextElementSibling,
							'[data-pc-section="menu"]',
					  )
					: null;
		...
		const onHeaderEnterKey = (event, item) => {
			const headerAction = DomHandler.findSingle(
				event.currentTarget,
		...
		const updateFocusedHeader = (event) => {
			...
			const panelElement = originalEvent.currentTarget.closest(
		...
		const changeActiveItem = (event, item) => {
			if (!isItemDisabled(item)) {
				...
				const _activeItemState =
					activeItemState && ObjectUtils.equals(item, activeItemState)
						? null
						: item;
				...
				if (props.multiple) {
					...
					if (
						...
						activeItems = activeItemsState.filter(
							(subItem) => !ObjectUtils.equals(item, subItem),
						);
					} else {
				...
				isExpanded && event
					? props.onOpen && props.onOpen({ originalEvent: event, item })
					: props.onClose && props.onClose({ originalEvent: event, item });
			}
		};

		const changeExpandedKeys = ({ item, expanded = false }) => {
			if (props.expandedKeys) {
				const _keys = { ...props.expandedKeys };
		...
		const createPanel = (item, index) => {
			...
			const submenuIcon =
				...
				IconUtils.getJSXIcon(
					active
						? props.submenuIcon || (
								<ChevronDownIcon {...headerSubmenuIconProps} />
						  )
						: props.submenuIcon || (
								<ChevronRightIcon {...headerSubmenuIconProps} />
						  ),
				);
			...
			const label = item.label && (
				<span {...headerLabelProps}>{item.label}</span>
			);
