...
export const PanelMenuList = React.memo((props) => {
    ...
    const onArrowDownKey = (event) => {
        const processedItem = ObjectUtils.isNotEmpty(focusedItem)
            ? findNextItem(focusedItem)
            : findFirstItem();
    ...
    const onArrowLeftKey = (event) => {
        if (ObjectUtils.isNotEmpty(focusedItem)) {
            const matched = activeItemPath.some(
                (p) => p.key === focusedItem.key,
            );

            if (matched) {
                setActiveItemPath(
                    activeItemPath.filter((p) => p.key !== focusedItem.key),
                );
            } else {
                setFocusedItem(
                    ObjectUtils.isNotEmpty(focusedItem.parent)
                        ? focusedItem.parent
                        : focusedItem,
                );
            }
            ...
            if (grouped) {
                ...
                if (matched) {
                    ...
                    const _activeItemPath = activeItemPath.filter(
                        (p) => p.parentKey !== focusedItem.parentKey,
                    );
    ...
    const isVisibleItem = (processedItem) => {
        return (
            !!processedItem &&
            (processedItem.level === 0 || isItemActive(processedItem)) &&
    ...
    const findLastItem = () => {
        return ObjectUtils.findLast(visibleItems, (processedItem) =>
            isValidItem(processedItem),
        );
    };
    ...
    const findNextItem = (processedItem) => {
        const index = visibleItems.findIndex(
            (item) => item.key === processedItem.key,
        );
        const matchedItem =
            index < visibleItems.length - 1
                ? visibleItems
                      .slice(index + 1)
                      .find((pItem) => isValidItem(pItem))
                : undefined;
    ...
    const findPrevItem = (processedItem) => {
        ...
        const matchedItem =
            index > 0
                ? ObjectUtils.findLast(visibleItems.slice(0, index), (pItem) =>
                      isValidItem(pItem),
                  )
                : undefined;
    ...
    const searchItems = (event, char) => {
        ...
        if (ObjectUtils.isNotEmpty(focusedItem)) {
            const focusedItemIndex = visibleItems.findIndex(
                (processedItem) => processedItem.key === focusedItem.key,
            );

            matchedItem = visibleItems
                .slice(focusedItemIndex)
                .find((processedItem) => isItemMatched(processedItem));
            ...
            matchedItem = visibleItems.find((processedItem) =>
        ...
        if (
            ObjectUtils.isEmpty(matchedItem) &&
            ObjectUtils.isEmpty(focusedItem)
        ) {
    ...
    const changeFocusedItem = (event) => {
        ...
        if (
            ObjectUtils.isNotEmpty(focusedItem) &&
            focusedItem.key !== processedItem.key
        ) {
            setFocusedItem(processedItem);
            scrollInView();
    ...
    const autoUpdateActiveItemPath = (expandedKeys) => {
        const _activeItemPath = Object.entries(expandedKeys || {}).reduce(
            (acc, [key, val]) => {
                if (val) {
    ...
    const findProcessedItemByItemKey = (key, processed, level = 0) => {
        const _processedItems = processed
            ? processed
            : level === 0 && props.model;
        ...
        for (let i = 0; i < _processedItems.length; i++) {
            ...
            const processedKey =
                getItemProp(processedItem, "key") || processedItem.key;

            if (processedKey === key) return processedItem;

            const matchedItem = findProcessedItemByItemKey(
                key,
                processedItem.items,
                level + 1,
            );
    ...
    useUpdateEffect(() => {
        const _id = ObjectUtils.isNotEmpty(focusedItem)
            ? `${props.panelId}_${focusedItem.key}`
            : null;
