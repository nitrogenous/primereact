...
export const PrimeReactProvider = (props) => {
	...
	const [inputStyle, setInputStyle] = useState(
		propsValue.inputStyle || "outlined",
	);
	...
	const [styleContainer, setStyleContainer] = useState(
		propsValue.styleContainer || null,
	);
	const [cssTransition, setCssTransition] = useState(
		propsValue.cssTransition || true,
	);
	const [autoZIndex, setAutoZIndex] = useState(propsValue.autoZIndex || true);
	const [hideOverlaysOnDocumentScrolling, setHideOverlaysOnDocumentScrolling] =
		useState(propsValue.hideOverlaysOnDocumentScrolling || false);
	const [nonce, setNonce] = useState(propsValue.nonce || null);
	const [nullSortOrder, setNullSortOrder] = useState(
		propsValue.nullSortOrder || 1,
	);
