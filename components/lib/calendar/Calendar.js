	...
	React.forwardRef((inProps, ref) => {
			...
			switch (event.code) {
				...
				case "Tab": {
					if (overlayRef && overlayRef.current) {
						DomHandler.getFocusableElements(overlayRef.current).forEach(
							(el) => (el.tabIndex = "-1"),
						);
						hide();
						props.touchUI && disableModality();
		...
		const isValidSelection = (value) => {
			...
			if (isSingleSelection()) {
				...
				value.every(
					(v) =>
						isSelectable(v.getDate(), v.getMonth(), v.getFullYear(), false) &&
						isSelectableTime(v),
				)
			) {
				if (isRangeSelection()) {
		...
		const incrementHour = (event) => {
			...
			if (validateHour(newHour, currentTime)) {
				if (
					props.maxDate &&
					props.maxDate.toDateString() === currentTime.toDateString() &&
					...
					if (props.maxDate.getMinutes() < currentTime.getMinutes()) {
						if (props.maxDate.getSeconds() < currentTime.getSeconds()) {
							if (
								props.maxDate.getMilliseconds() < currentTime.getMilliseconds()
							) {
								updateTime(
									event,
									newHour,
		...
		const decrementHour = (event) => {
			...
			if (validateHour(newHour, currentTime)) {
				if (
					props.minDate &&
					props.minDate.toDateString() === currentTime.toDateString() &&
					...
					if (props.minDate.getMinutes() > currentTime.getMinutes()) {
						if (props.minDate.getSeconds() > currentTime.getSeconds()) {
							if (
								props.minDate.getMilliseconds() > currentTime.getMilliseconds()
							) {
								updateTime(
									event,
									newHour,
		...
		const toggleAmPm = (event) => {
			...
			if (
				validateHour(convertTo24Hour(newHour, !(currentHour > 11)), currentTime)
			) {
				updateTime(
					event,
					newHour,
		...
		const getViewDate = (date) => {
			...
			const viewDate =
				date || (props.onViewDateChange ? props.viewDate : viewDateState);
		...
		const validateDate = (value) => {
			...
			if (renderMonthsNavigator(0)) {
				...
				const viewMonthWithMinMax = parseInt(
					(isInMinYear(value) &&
						Math.max(props.minDate.getMonth(), viewMonth).toString()) ||
						(isInMaxYear(value) &&
							Math.min(props.maxDate.getMonth(), viewMonth).toString()) ||
						viewMonth,
				);
		...
		const onDateCellKeydown = (event, date, groupIndex) => {
			...
			switch (event.code) {
				case "ArrowDown": {
					...
					if (nextRow) {
						...
						if (hasNextFocusableDate) {
							const focusCell =
								hasNextFocusableDate.children[cellIndex].children[0];
				...
				case "End": {
					...
					const currentRow = cell.parentElement;
					const focusCell =
						currentRow.children[currentRow.children.length - 1].children[0];
		...
		const onYearCellKeydown = (event, index) => {
			...
			switch (event.code) {
				...
				case "ArrowDown": {
					...
					const nextCell =
						cells[event.code === "ArrowDown" ? cellIndex + 2 : cellIndex - 2];
		...
		const onMonthSelect = (event, month) => {
			if (props.view === "month") {
				...
				setCurrentView("date");
				props.onMonthChange &&
					props.onMonthChange({ month: month + 1, year: currentYear });
		...
		const onYearSelect = (event, year) => {
			if (props.view === "year") {
				...
				setCurrentView("month");
				props.onMonthChange &&
					props.onMonthChange({ month: currentMonth + 1, year: year });
			}
		};
				...
				if (isSingleSelection()) {
					...
					if (props.value[1])
						return (
							...
							isDateBetween(props.value[0], props.value[1], dateMeta)
						);
					else {
		...
		const isMonthYearDisabled = (month, year) => {
			const daysCountInAllMonth =
				month === -1
					? new Array(12).fill(0).map((_, i) => getDaysCountInMonth(i, year))
		...
		const isInMinYear = (viewDate) => {
			return (
				props.minDate && props.minDate.getFullYear() === viewDate.getFullYear()
			);
		};

		const isInMaxYear = (viewDate) => {
			return (
				props.maxDate && props.maxDate.getFullYear() === viewDate.getFullYear()
			);
		};
		...
		const createTitleMonthElement = (month, monthIndex) => {
			...
			if (renderMonthsNavigator(monthIndex)) {
				...
				const displayedMonthOptions = monthNames
					.map((month, index) =>
						(!isInMinYear(viewDate) || index >= props.minDate.getMonth()) &&
						(!isInMaxYear(viewDate) || index <= props.maxDate.getMonth())
				...
				return (
					<span {...decadeTitleProps}>
						{props.decadeTemplate ? (
							...
							<span {...decadeTitleTextProps}>{`${yearPickerValues()[0]} - ${
			...
			const week = weekDates.map((date) => {
				...
				const content =
					date.otherMonth && !props.showOtherMonths
						? null
						: createDateCellContent(date, dateClassName, groupIndex);
		...
		const createMonth = (monthMetaData, index) => {
			...
			const forwardNavigator = createForwardNavigator(
				props.numberOfMonths === 1 || index === props.numberOfMonths - 1,
			);
		...
		const rootProps = mergeProps(
			{
				...
				className: classNames(
					props.className,
					cx("root", { focusedState, isFilled, panelVisible: visible }),
				),
				style: props.style,
			},
