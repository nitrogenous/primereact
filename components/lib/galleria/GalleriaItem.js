...
export const GalleriaItem = React.memo(
	React.forwardRef((props, ref) => {
		...
		const createBackwardNavigator = () => {
			if (props.showItemNavigators) {
				...
				const icon = props.itemPrevIcon || (
					<ChevronLeftIcon {...previousItemIconProps} />
				);
				const itemPrevIcon = IconUtils.getJSXIcon(
					icon,
					{ ...previousItemIconProps },
					{ props },
				);
		...
		const createForwardNavigator = () => {
			if (props.showItemNavigators) {
				const isDisabled =
					!props.circular && props.activeItemIndex === props.value.length - 1;
				...
				const icon = props.itemNextIcon || (
					<ChevronRightIcon {...nextItemIconProps} />
				);
				const itemNextIcon = IconUtils.getJSXIcon(
					icon,
					{ ...nextItemIconProps },
					{ props },
				);
