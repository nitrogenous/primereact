...
export const Galleria = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [slideShowActiveState, setSlideShowActiveState] =
		...
		const activeItemIndex = props.onItemChange
			? props.activeIndex
			: activeIndexState;
		const isVertical =
		...
		useInterval(
			() => {
				onActiveItemChange({
					index:
						props.circular && props.value.length - 1 === activeItemIndex
							? 0
							: activeItemIndex + 1,
				});
			},
		...
		const createElement = () => {
			const thumbnailsPosClassName =
				props.showThumbnails &&
				getPositionClassName("p-galleria-thumbnails", props.thumbnailsPosition);
			const indicatorPosClassName =
				props.showIndicators &&
				getPositionClassName("p-galleria-indicators", props.indicatorsPosition);
