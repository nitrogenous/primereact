...
const GalleriaThumbnailItem = React.memo((props) => {
	...
	const onRightKey = () => {
		const indicators = DomHandler.find(
			props.itemsContainerRef.current,
	...
	const onTabKey = () => {
		...
		const highlightedIndex = indicators.findIndex(
			(ind) => DomHandler.getAttribute(ind, "data-p-active") === true,
		);
...
export const GalleriaThumbnails = React.memo(
	React.forwardRef((props, ref) => {
		...
		const step = (dir) => {
			...
			if (
				dir < 0 &&
				-1 * totalShiftedItems + numVisibleState > props.value.length - 1
			) {
			...
			if (itemsContainerRef.current) {
				...
				itemsContainerRef.current.style.transform = props.isVertical
		...
		const navBackward = (e) => {
			...
			const prevItemIndex =
				props.activeItemIndex !== 0 ? props.activeItemIndex - 1 : 0;
			const diff = prevItemIndex + totalShiftedItemsState;

			if (
				numVisibleState - diff - 1 > getMedianItemIndex() &&
				(-1 * totalShiftedItemsState !== 0 || props.circular)
			) {
				step(1);
			}
			...
			props.onActiveItemChange({
				index:
					props.circular && props.activeItemIndex === 0
		...
		const navForward = (e) => {
			...
			if (
				nextItemIndex + totalShiftedItemsState > getMedianItemIndex() &&
				(-1 * totalShiftedItemsState < getTotalPageNumber() - 1 ||
					props.circular)
			) {
				step(-1);
			}
			...
			props.onActiveItemChange({
				index:
					props.circular && props.value.length - 1 === props.activeItemIndex
						? 0
						: nextItemIndex,
			});
		...
		const onItemClick = (event) => {
			...
			const selectedItemIndex = event.index;
		...
		const getTotalPageNumber = () => {
			return props.value.length > numVisibleState
				? props.value.length - numVisibleState + 1
				: 0;
		};
			...
			if (props.responsiveOptions) {
				...
				for (let i = 0; i < responsiveOptions.current.length; i++) {
                    ...
                    @media screen and (max-width: ${res.breakpoint}) {
                        [data-pc-section="thumbnailitems"][${
		...
		useUpdateEffect(() => {
			...
			if (
				...
				prevActiveItemIndex !== props.activeItemIndex
			) {
				if (props.activeItemIndex <= getMedianItemIndex()) {
					totalShiftedItems = 0;
				} else if (
					props.value.length - numVisibleState + getMedianItemIndex() <
					props.activeItemIndex
				) {
					totalShiftedItems = numVisibleState - props.value.length;
				} else if (
					props.value.length - numVisibleState < props.activeItemIndex &&
					numVisibleState % 2 === 0
				) {
					totalShiftedItems =
						props.activeItemIndex * -1 + getMedianItemIndex() + 1;
				} else {
		...
		const createBackwardNavigator = () => {
			if (props.showThumbnailNavigators) {
				...
				const icon = props.isVertical
					? props.prevThumbnailIcon || (
							<ChevronUpIcon {...previousThumbnailIconProps} />
					  )
					: props.prevThumbnailIcon || (
							<ChevronLeftIcon {...previousThumbnailIconProps} />
					  );
				const prevThumbnailIcon = IconUtils.getJSXIcon(
					icon,
					{ ...previousThumbnailIconProps },
					{ props },
				);
		...
		const createForwardNavigator = () => {
			if (props.showThumbnailNavigators) {
				const isDisabled =
					(!props.circular &&
						props.activeItemIndex === props.value.length - 1) ||
					props.value.length <= numVisibleState;
				...
				const icon = props.isVertical
					? props.nextThumbnailIcon || (
							<ChevronDownIcon {...nextThumbnailIconProps} />
					  )
					: props.nextThumbnailIcon || (
							<ChevronRightIcon {...nextThumbnailIconProps} />
					  );
				const nextThumbnailIcon = IconUtils.getJSXIcon(
					icon,
					{ ...nextThumbnailIconProps },
					{ props },
				);
