...
const globalEscKeyHandlingLogic = {
	...
	onGlobalKeyDown(event) {
		...
		const theMostImportantEscHandlersSet =
			escKeyListeners.get(maxPrimaryPriority);
		const maxSecondaryPriority = Math.max(
			...theMostImportantEscHandlersSet.keys(),
		);
		const theMostImportantEscHandler =
