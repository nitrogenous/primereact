	...
	React.forwardRef((inProps, ref) => {
		...
		const formatValue = (value) => {
			if (value) {
				const field = Array.isArray(props.field)
					? props.field[props.trigger.findIndex((f) => f === triggerState.key)]
					: props.field;
		...
		const onKeyDown = (event) => {
			if (overlayVisibleState) {
				...
				switch (event.which) {
					...
					case 40:
						if (highlightItem) {
							const nextElement = highlightItem.nextElementSibling;
					...
					case 38:
						if (highlightItem) {
							const previousElement = highlightItem.previousElementSibling;
					...
					case 13:
						if (highlightItem) {
							selectItem(
								event,
								props.suggestions[DomHandler.index(highlightItem)],
							);
						}
		...
		useUpdateEffect(() => {
			...
			const isInputWrapperFilled = _isUnstyled
				...
				: DomHandler.hasClass(elementRef.current, "p-inputwrapper-filled");
