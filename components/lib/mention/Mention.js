    ...
    React.forwardRef((inProps, ref) => {
        ...
        const alignOverlay = () => {
            if (triggerState) {
                ...
                const position = DomHandler.getCursorOffset(
                    inputRef.current,
        ...
        const getLatestTrigger = (value, start) => {
            if (Array.isArray(props.trigger)) {
                ...
                props.trigger.forEach((t) => {
                    ...
                    if (
                        index !== -1 &&
                        (index > latestTrigger.index || !latestTrigger.index)
                    ) {
                        latestTrigger = {
                            key: t,
        ...
        const selectItem = (event, suggestion) => {
            ...
            const currentText = value.substring(
                triggerState.index,
                spaceIndex > -1 ? spaceIndex : selectionStart,
            );
            ...
            if (currentText.trim() !== selectedText) {
                ...
                const nextText = value.substring(
                    spaceIndex > -1
                        ? selectionStart
                        : triggerState.index + currentText.length,
                );
        ...
        const formatValue = (value) => {
            if (value) {
                const field = Array.isArray(props.field)
                    ? props.field[
                          props.trigger.findIndex((f) => f === triggerState.key)
                      ]
                    : props.field;

                return field
                    ? ObjectUtils.resolveFieldData(value, field)
                    : value;
            }
                        ...
                        if (highlightItem) {
                            ...
                            if (nextElement) {
                                const nextElementIndex =
                                    DomHandler.index(nextElement);
                                const highlightItemIndex =
                                    DomHandler.index(highlightItem);
                        ...
                        if (highlightItem) {
                            ...
                            if (previousElement) {
                                const previousElementIndex =
                                    DomHandler.index(previousElement);
                                const highlightItemIndex =
                                    DomHandler.index(highlightItem);
        ...
        useUpdateEffect(() => {
            ...
            const isInputWrapperFilled = _isUnstyled
                ? DomHandler.isAttributeEquals(
                      elementRef.current,
                      "data-p-inputwrapper-filled",
                      true,
                  )
                : DomHandler.hasClass(
                      elementRef.current,
        ...
        const createPanel = () => {
            const header = ObjectUtils.getJSXElement(
                props.headerTemplate,
                props,
            );
            const footer = ObjectUtils.getJSXElement(
