	...
	React.forwardRef((inProps, ref) => {
		...
		const setValue = (event) => {
			...
			if (props.step) {
				...
				if (diff < 0)
					newValue =
						oldValue +
						Math.ceil(newValue / props.step - oldValue / props.step) *
							props.step;
				else if (diff > 0)
					newValue =
						oldValue +
						Math.floor(newValue / props.step - oldValue / props.step) *
							props.step;
			} else {
		...
		const createRangeSlider = () => {
			const handleValueStart =
				((value[0] < props.min ? props.min : value[0] - props.min) * 100) /
				(props.max - props.min);
			const handleValueEnd =
				((value[1] > props.max ? props.max : value[1] - props.min) * 100) /
				(props.max - props.min);
			...
			const rangeSliderPosition =
				handleValueEnd > handleValueStart ? handleValueStart : handleValueEnd;
