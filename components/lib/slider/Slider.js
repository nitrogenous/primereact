...
export const Slider = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const [bindDocumentTouchMoveListener, unbindDocumentTouchMoveListener] =
            useEventListener({
                type: "touchmove",
                listener: (event) => onDrag(event),
            });
        const [bindDocumentTouchEndListener, unbindDocumentTouchEndListener] =
            useEventListener({
                type: "touchend",
                listener: (event) => onDragEnd(event),
            });
        ...
        const setValue = (event) => {
            ...
            const pageX = ObjectUtils.isNotEmpty(event.touches)
                ? event.touches[0].pageX
                : event.pageX;
            const pageY = ObjectUtils.isNotEmpty(event.touches)
            ...
            if (horizontal)
                handleValue =
                    ((pageX - initX.current) * 100) / barWidth.current;
            else
                handleValue =
                    ((initY.current + barHeight.current - pageY) * 100) /
                    barHeight.current;

            let newValue =
                (props.max - props.min) * (handleValue / 100) + props.min;

            if (props.step) {
                const oldValue = props.range
                    ? value[handleIndex.current]
                    : value;
                ...
                if (diff < 0)
                    newValue =
                        oldValue +
                        Math.ceil(
                            newValue / props.step - oldValue / props.step,
                        ) *
                            props.step;
                else if (diff > 0)
                    newValue =
            ...
            const handleProps = mergeProps(
                {
                    ...
                    style: {
                        ...sx("handle", { dragging, leftValue, bottomValue }),
                        ...style,
                    },
        ...
        const createRangeSlider = () => {
            const handleValueStart =
                ((value[0] < props.min ? props.min : value[0] - props.min) *
                    100) /
                (props.max - props.min);
            const handleValueEnd =
                ((value[1] > props.max ? props.max : value[1] - props.min) *
                    100) /
                (props.max - props.min);

            const rangeStartHandle = horizontal
                ? createHandle(handleValueStart, null, 0)
                : createHandle(null, handleValueStart, 0);
            const rangeEndHandle = horizontal
                ? createHandle(handleValueEnd, null, 1)
                : createHandle(null, handleValueEnd, 1);
            const rangeSliderWidth =
                handleValueEnd > handleValueStart
                    ? handleValueEnd - handleValueStart
                    : handleValueStart - handleValueEnd;
            const rangeSliderPosition =
                handleValueEnd > handleValueStart
                    ? handleValueStart
                    : handleValueEnd;
