...
export const Carousel = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [totalShiftedItemsState, setTotalShiftedItemsState] = React.useState(
			props.page * props.numScroll * -1,
		);
		...
		const totalIndicators = props.value
			? Math.max(
					Math.ceil((props.value.length - numVisibleState) / numScrollState) +
						1,
					0,
			  )
			: 0;
			...
			if (itemsContainerRef.current && responsiveOptions.current) {
				...
				if (numScrollState !== matchedResponsiveData.numScroll) {
					const page = Math.floor(
						(currentPage * numScrollState) / matchedResponsiveData.numScroll,
					);
		...
		const onTabKey = () => {
			...
			const highlightedIndex = indicators.findIndex(
				(ind) => DomHandler.getAttribute(ind, "data-p-highlight") === true,
			);
		...
		const changePosition = (totalShiftedItems) => {
			if (itemsContainerRef.current) {
				itemsContainerRef.current.style.transform = isVertical
					? `translate3d(0, ${totalShiftedItems * (100 / numVisibleState)}%, 0)`
					: `translate3d(${
		...
		const ariaSlideNumber = (value) => {
			return localeOption("aria")
				? localeOption("aria").slideNumber.replace(/{slideNumber}/g, value)
				: undefined;
		};
		...
		const createItems = () => {
			if (props.value && props.value.length) {
				...
				const items = props.value.map((item, index) => {
					...
					const ariaHidden =
						firstIndex > index || lastIndex < index ? true : undefined;
		...
		const createBackwardNavigator = () => {
			if (props.showNavigators) {
				const isDisabled =
					(!circular ||
						(props.value && props.value.length < numVisibleState)) &&
					currentPage === 0;
				...
				const icon = isVertical
					? props.prevIcon || <ChevronUpIcon {...previousButtonIconProps} />
					: props.prevIcon || <ChevronLeftIcon {...previousButtonIconProps} />;
		...
		const createForwardNavigator = () => {
			if (props.showNavigators) {
				const isDisabled =
					(!circular ||
						(props.value && props.value.length < numVisibleState)) &&
					(currentPage === totalIndicators - 1 || totalIndicators === 0);
				...
				const icon = isVertical
					? props.nextIcon || <ChevronDownIcon {...nextButtonIconProps} />
					: props.nextIcon || <ChevronRightIcon {...nextButtonIconProps} />;
