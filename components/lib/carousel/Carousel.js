...
const CarouselItem = React.memo((props) => {
    const { ptm, cx } = props;
    const key =
        props.className && props.className === "p-carousel-item-cloned"
            ? "itemCloned"
            : "item";
...
export const Carousel = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const [numVisibleState, setNumVisibleState] = React.useState(
            props.numVisible,
        );
        const [numScrollState, setNumScrollState] = React.useState(
            props.numScroll,
        );
        const [totalShiftedItemsState, setTotalShiftedItemsState] =
            React.useState(props.page * props.numScroll * -1);
        ...
        const isCircular =
            circular && props.value && props.value.length >= numVisibleState;
        const totalIndicators = props.value
            ? Math.max(
                  Math.ceil(
                      (props.value.length - numVisibleState) / numScrollState,
                  ) + 1,
                  0,
              )
            : 0;
        const isAutoplay =
            totalIndicators && props.autoplayInterval && allowAutoplay.current;
        ...
        const step = (dir, page) => {
            ...
            if (page != null) {
                ...
                if (isRemainingItemsAdded.current) {
                    totalShiftedItems +=
                        remainingItems.current - numScrollState * dir;
                    isRemainingItemsAdded.current = false;
                }

                const originalShiftedItems = isCircular
                    ? totalShiftedItems + numVisibleState
                    : totalShiftedItems;

                page = Math.abs(
                    Math.floor(originalShiftedItems / numScrollState),
                );
            }
            ...
            if (isCircular && pageState === totalIndicators - 1 && dir === -1) {
                ...
                page === totalIndicators - 1 &&
                remainingItems.current > 0
            ) {
                totalShiftedItems +=
                    remainingItems.current * -1 - numScrollState * dir;
        ...
        const calculatePosition = () => {
            if (itemsContainerRef.current && responsiveOptions.current) {
                ...
                for (let i = 0; i < responsiveOptions.current.length; i++) {
                    const res = responsiveOptions.current[i];
                ...
                if (numScrollState !== matchedResponsiveData.numScroll) {
                    const page = Math.floor(
                        (currentPage * numScrollState) /
                            matchedResponsiveData.numScroll,
                    );
                    let totalShiftedItems =
                        matchedResponsiveData.numScroll * page * -1;
        ...
        const onTransitionEnd = (e) => {
            if (itemsContainerRef.current && e.propertyName === "transform") {
                ...
                if (
                    (pageState === 0 || pageState === totalIndicators - 1) &&
                    isCircular
                ) {
        ...
        const onTabKey = () => {
            ...
            const highlightedIndex = indicators.findIndex(
                (ind) =>
                    DomHandler.getAttribute(ind, "data-p-highlight") === true,
            );
            ...
            const activeIndex = indicators.findIndex(
                (ind) => ind === activeIndicator.parentElement,
            );
        ...
        const destroyStyle = () => {
            carouselStyle.current = DomHandler.removeInlineStyle(
                carouselStyle.current,
            );
        };

        const changePosition = (totalShiftedItems) => {
            if (itemsContainerRef.current) {
                itemsContainerRef.current.style.transform = isVertical
                    ? `translate3d(0, ${
                          totalShiftedItems * (100 / numVisibleState)
                      }%, 0)`
                    : `translate3d(${
                          totalShiftedItems * (100 / numVisibleState)
                      }%, 0, 0)`;
        ...
        useUpdateEffect(() => {
            ...
            if (
                prevNumScroll !== numScrollState ||
                prevNumVisible !== numVisibleState ||
                (props.value &&
                    prevValue &&
                    prevValue.length !== props.value.length)
            ) {
                remainingItems.current =
                    (props.value.length - numVisibleState) % numScrollState;
                ...
                if (
                    ...
                    totalShiftedItems +=
                        -1 * remainingItems.current + numScrollState;
                    isRemainingItemsAdded.current = true;
                } else {
                    isRemainingItemsAdded.current = false;
                }
        ...
        const ariaSlideNumber = (value) => {
            return localeOption("aria")
                ? localeOption("aria").slideNumber.replace(
                      /{slideNumber}/g,
                      value,
                  )
                : undefined;
        ...
        const createItems = () => {
            if (props.value && props.value.length) {
                ...
                if (isCircular) {
                    ...
                    clonedItemsForStarting = clonedElements.map(
                        (item, index) => {
                            const isActive =
                                totalShiftedItemsState * -1 ===
                    ...
                    clonedItemsForFinishing = clonedElements.map(
                        (item, index) => {
                ...
                const items = props.value.map((item, index) => {
                    const firstIndex = isCircular
                        ? -1 * (totalShiftedItemsState + numVisibleState)
                        : totalShiftedItemsState * -1;
                    ...
                    const ariaRoledescription = localeOption("aria")
                        ? localeOption("aria").slide
                        : undefined;
        ...
        const createBackwardNavigator = () => {
            if (props.showNavigators) {
                ...
                const icon = isVertical
                    ? props.prevIcon || (
                          <ChevronUpIcon {...previousButtonIconProps} />
                      )
                    : props.prevIcon || (
                ...
                const previousButtonProps = mergeProps(
                    {
                        ...
                        disabled: isDisabled,
                        "aria-label": localeOption("aria")
                            ? localeOption("aria").previousPageLabel
                            : undefined,
        ...
        const createForwardNavigator = () => {
            if (props.showNavigators) {
                ...
                const icon = isVertical
                    ? props.nextIcon || (
                          <ChevronDownIcon {...nextButtonIconProps} />
                      )
                    : props.nextIcon || (
                ...
                const nextButtonProps = mergeProps(
                    {
                        ...
                        disabled: isDisabled,
                        "aria-label": localeOption("aria")
                            ? localeOption("aria").nextPageLabel
                            : undefined,
                        "data-pc-group-section": "navigator",
                    },
                    ptm("nextButton"),
        ...
        const ariaPageLabel = (value) => {
            return localeOption("aria")
                ? localeOption("aria").pageLabel.replace(/{page}/g, value)
                : undefined;
        };
                ...
                const indicatorsProps = mergeProps(
                    {
                        ...
                        className: classNames(
                            props.indicatorsContentClassName,
                            cx("indicators"),
                        ),
