...
export const Menubar = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [bindOutsideClickListener, unbindOutsideClickListener] =
			useEventListener({
				type: "click",
				listener: (event) => {
		...
		const isOutsideClicked = (event) => {
			return (
				rootMenuRef.current !== event.target &&
				!rootMenuRef.current.contains(event.target) &&
		...
		const getProccessedItemLabel = (processedItem) =>
			processedItem ? getItemLabel(processedItem.item) : undefined;

		const isProccessedItemGroup = (processedItem) =>
			processedItem && ObjectUtils.isNotEmpty(processedItem.items);
		...
		const onFocus = (event) => {
			...
			setFocusedItemInfo(
				focusedItemInfo.index !== -1
					? focusedItemInfo
					: { index: findFirstFocusedItemIndex(), level: 0, parentKey: "" },
			);
			...
			if (selected) {
				...
				if (grouped) {
					...
					const rootProcessedItem = root
						? processedItem
						: activeItemPath.find((p) => p.parentKey === "");
					const rootProcessedItemIndex = rootProcessedItem
						? rootProcessedItem.index
						: -1;
		...
		const onArrowDownKey = (event) => {
			...
			const root = processedItem
				? ObjectUtils.isEmpty(processedItem.parent)
				: null;
		...
		const onArrowUpKey = (event) => {
			...
			if (root) {
				...
				const parentItem = activeItemPath.find(
					(p) => p.key === processedItem.parentKey,
				);
		...
		const onArrowLeftKey = (event) => {
			...
			const parentItem = processedItem
				? activeItemPath.find((p) => p.key === processedItem.parentKey)
				: null;

			if (parentItem) {
				onItemChange({ originalEvent: event, processedItem: parentItem });
				setActiveItemPath(
					activeItemPath.filter((p) => p.key !== parentItem.key),
				);
			} else {
				const itemIndex =
					focusedItemInfo.index !== -1
		...
		const createProcessedItems = React.useCallback(
		...
		useUpdateEffect(() => {
			...
			const _processedItems = processedItem
				? processedItem.items
				: processedItems;
			...
			const buttonProps = mergeProps(
				{
					...
					tabIndex: "0",
					"aria-haspopup":
						mobileActiveState && props.model && props.model.lentgh > 0
							? true
							: false,
