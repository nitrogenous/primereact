...
export const FocusTrap = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const setAutoFocus = (target) => {
			...
			let focusableElement = DomHandler.getFirstFocusableElement(
				target,
			...
			autoFocus &&
				!focusableElement &&
				(focusableElement = DomHandler.getFirstFocusableElement(
					target,
					getComputedSelector(firstFocusableSelector),
				));
		...
		const getComputedSelector = (selector) => {
			return `:not(.p-hidden-focusable):not([data-p-hidden-focusable="true"])${
				selector ?? ""
			}`;
		};
			...
			const focusableElement =
				...
				!targetRef.current?.contains(relatedTarget)
					? DomHandler.getFirstFocusableElement(
							...
							getComputedSelector(currentTarget.$_pfocustrap_focusableselector),
					  )
					: currentTarget.$_pfocustrap_lasthiddenfocusableelement;
		...
		const createHiddenFocusableElements = () => {
			...
			const firstFocusableElement = createFocusableElement(
				onFirstHiddenElementFocus,
				"firstfocusableelement",
			);
			const lastFocusableElement = createFocusableElement(
				onLastHiddenElementFocus,
				"lastfocusableelement",
			);

			if (
				firstFocusableElement.ref.current &&
				lastFocusableElement.ref.current
			) {
				firstFocusableElement.ref.current.$_pfocustrap_lasthiddenfocusableelement =
					lastFocusableElement.ref.current;
				lastFocusableElement.ref.current.$_pfocustrap_firsthiddenfocusableelement =
					firstFocusableElement.ref.current;
			}
