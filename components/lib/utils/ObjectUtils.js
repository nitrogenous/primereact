export default class ObjectUtils {
		...
		if (child) {
			...
			try {
				if (process.env.NODE_ENV !== "production" && !isValid) {
					...
					console.error(
						`PrimeReact: Unexpected type; '${childType}'. Parent component expects a ${messageTypes
							.map((t) => `${t}`)
							.join(" or ")} component or a component with the ${messageTypes
							.map((t) => `__TYPE="${t}"`)
	...
	static isObject(value) {
		return (
			value !== null && value instanceof Object && value.constructor === Object
		);
	}
