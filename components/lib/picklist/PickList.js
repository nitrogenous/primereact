...
export const PickList = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [sourceFilterValueState, setSourceFilterValueState] =
			React.useState("");
		const [targetFilterValueState, setTargetFilterValueState] =
			React.useState("");
		const [attributeSelectorState, setAttributeSelectorState] = React.useState(
			props.id,
		);
		...
		const sourceSelection = props.sourceSelection
			? props.sourceSelection
			: sourceSelectionState;
		const targetSelection = props.targetSelection
			? props.targetSelection
			: targetSelectionState;
		const sourceFilteredValue = props.onSourceFilterChange
			? props.sourceFilterValue
			: sourceFilterValueState;
		const targetFilteredValue = props.onTargetFilterChange
			? props.targetFilterValue
			: targetFilterValueState;
		...
		const onTransfer = (event) => {
			...
			onSelectionChange(
				{ originalEvent, value: selectedValue },
				"sourceSelection",
			...
			onSelectionChange(
				{ originalEvent, value: selectedValue },
				"targetSelection",
				props.onTargetSelectionChange,
			);
			handleChange(event, source, target);
		};

		const scrollInView = (listContainer, direction = 1) => {
			const selectedItems = listContainer.getElementsByClassName("p-highlight");
		...
		const onSelectionChange = (e, stateKey, callback) => {
			...
			if (
				ObjectUtils.isNotEmpty(sourceSelection) &&
				stateKey === "targetSelection"
			) {
				setSourceSelectionState([]);
			} else if (
				ObjectUtils.isNotEmpty(targetSelection) &&
				stateKey === "sourceSelection"
			) {
		...
		const onFilter = (event) => {
			...
			const [setFilterState, onFilterChange] =
				type === "source"
		...
		const getVisibleList = (list, type) => {
			const [filteredValue, filterCallback] =
				type === "source"
					? [sourceFilteredValue, filterSource]
					: [targetFilteredValue, filterTarget];

			return hasFilterBy && ObjectUtils.isNotEmpty(filteredValue)
				? filterCallback(filteredValue)
				: list;
		};
		...
		const onListFocus = (event, type) => {
			...
			const selectedFirstItem =
				DomHandler.findSingle(listElement, '[data-p-highlight="true"]') ||
				DomHandler.findSingle(listElement, '[data-pc-section="item"]');
			const itemList =
				listElement && listElement.children ? [...listElement.children] : [];
		...
		const onItemClick = (event, type, arrowKeyClick = false) => {
			...
			const index = ObjectUtils.findIndexInList(item, selection, props.dataKey);
			...
			if (isSource) {
				onSelectionChange(
					{ originalEvent: originalEvent, value: selection },
					"sourceSelection",
					props.onSourceSelectionChange,
				);
			} else {
			...
			switch (event.code) {
				...
				case "KeyA":
					if (event.ctrlKey) {
						...
						onSelectionChange(
							{ originalEvent: event, value: [...sourceList] },
							isSource ? "sourceSelection" : "targetSelection",
							isSource
								? props.onSourceSelectionChange
								: props.onTargetSelectionChange,
						);
		...
		const onArrowDownKey = (event, type) => {
			...
			const visibleList = getVisibleList(
				type === "source" ? props.source : props.target,
				type,
			);
			...
			if (visibleList && visibleList.length > 0 && event.shiftKey) {
				onItemClick(
					{ originalEvent: event, value: visibleList[optionIndex] },
					type,
					true,
				);
			}
		...
		const onEnterKey = (event, type) => {
			...
			if (visibleList && visibleList.length > 0) {
				onItemClick(
					{ originalEvent: event, value: visibleList[matchedOptionIndex], id },
					type,
				);
			}
		...
		const onSpaceKey = (event, type) => {
			...
			if (event.shiftKey && selection && selection.length > 0) {
				...
				const listElement = getListElement(type);
				const items = DomHandler.find(listElement, '[data-pc-section="item"]');
				const selectedItemIndex = ObjectUtils.findIndexInList(selection[0], [
					...listItems,
				]);
				const focusedItem = DomHandler.findSingle(
					listElement,
				...
				if (isSource) {
					onSelectionChange(
						{ originalEvent: event, value: selection },
						"sourceSelection",
						props.onSourceSelectionChange,
					);
				} else {
		...
		const onEndKey = (event, type) => {
			...
			if (event.ctrlKey && event.shiftKey) {
				...
				const selection = [...listItems].slice(
					matchedOptionIndex,
					items.length,
				);
		...
		const changeFocusedOptionIndex = (index, type) => {
			...
			const order =
				index >= items.length ? items.length - 1 : index < 0 ? 0 : index;
		...
		const getListElement = (type) => {
			return type === "source"
				? sourceListElementRef.current.getElement()
				: targetListElementRef.current.getElement();
		};
...
`;
		...
		const destroyStyle = () => {
			styleElementRef.current = DomHandler.removeInlineStyle(
		...
		useUpdateEffect(() => {
			const _focusedOptionId =
				focusedOptionIndex !== -1 ? focusedOptionIndex : null;

			setFocusedOptionId(_focusedOptionId);
		}, [focusedOptionIndex]);

		const sourceItemTemplate = props.sourceItemTemplate
			? props.sourceItemTemplate
			: props.itemTemplate;
		const targetItemTemplate = props.targetItemTemplate
			? props.targetItemTemplate
			: props.itemTemplate;
