...
export const PickList = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const [targetFilterValueState, setTargetFilterValueState] =
            React.useState("");
        const [attributeSelectorState, setAttributeSelectorState] =
            React.useState(props.id);
        ...
        const scrollInView = (listContainer, direction = 1) => {
            const selectedItems =
                listContainer.getElementsByClassName("p-highlight");
        ...
        const filterSource = (value = "") => {
            const filteredValue = value
                .trim()
                .toLocaleLowerCase(props.filterLocale);
        ...
        const onItemClick = (event, type, arrowKeyClick = false) => {
            ...
            const index = ObjectUtils.findIndexInList(
                item,
                selection,
                props.dataKey,
            );
        ...
        const onSpaceKey = (event, type) => {
            ...
            if (event.shiftKey && selection && selection.length > 0) {
                ...
                const selectedItemIndex = ObjectUtils.findIndexInList(
                    selection[0],
                    [...listItems],
                );
                const focusedItem = DomHandler.findSingle(
