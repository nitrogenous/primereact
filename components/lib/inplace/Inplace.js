...
export const Inplace = React.forwardRef((inProps, ref) => {
	...
	const createCloseButton = () => {
		...
		const closeAriaLabel = localeOption("aria")
	...
	const createChildren = () => {
		...
		return React.Children.map(props.children, (child) => {
			if (
				active &&
				ObjectUtils.isValidChild(child, "InplaceContent", validChildTypes)
			) {
				return createContent(child);
			} else if (
				!active &&
				ObjectUtils.isValidChild(child, "InplaceDisplay", validChildTypes)
			) {
