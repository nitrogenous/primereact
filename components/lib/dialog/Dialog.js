...
export const Dialog = React.forwardRef((inProps, ref) => {
    ...
    const shouldBlockScroll =
        visibleState && (props.blockScroll || (props.maximizable && maximized));
    ...
    const onKeyDown = (event) => {
        ...
        const dialogId = params[paramLength - 1]
    ...
    const updateGlobalDialogsRegistry = (isMounted) => {
        ...
        if (isMounted && visibleState) {
            ...
            const currentDialogIndexInRegistry =
                document.primeDialogParams.findIndex(
            ...
            if (currentDialogIndexInRegistry === -1) {
                ...
                document.primeDialogParams =
                    document.primeDialogParams.toSpliced(
    ...
    const createCloseIcon = () => {
        if (props.closable) {
            ...
            const icon = props.closeIcon || (
                <TimesIcon {...closeButtonIconProps} />
            );
            const headerCloseIcon = IconUtils.getJSXIcon(
                icon,
    ...
    const createMaximizeIcon = () => {
        ...
        if (!maximized) {
            icon = props.maximizeIcon || (
                <WindowMaximizeIcon {...maximizableIconProps} />
            );
        } else {
            icon = props.minimizeIcon || (
                <WindowMinimizeIcon {...maximizableIconProps} />
            );
        }

        const toggleIcon = IconUtils.getJSXIcon(icon, maximizableIconProps, {
            props,
        });
