...
export const StyleClass = React.forwardRef((inProps, ref) => {
    ...
    const [bindTargetEnterListener, unbindTargetEnterListener] =
        useEventListener({
            type: "animationend",
            listener: () => {
                DomHandler.removeClass(
                    targetRef.current,
                    props.enterActiveClassName,
                );

                if (props.enterToClassName) {
                    DomHandler.addClass(
                        targetRef.current,
                        props.enterToClassName,
                    );
                }
    ...
    const [bindTargetLeaveListener, unbindTargetLeaveListener] =
        useEventListener({
            type: "animationend",
            listener: () => {
                DomHandler.removeClass(
                    targetRef.current,
                    props.leaveActiveClassName,
                );

                if (props.leaveToClassName) {
                    DomHandler.addClass(
                        targetRef.current,
                        props.leaveToClassName,
                    );
                }
    ...
    const [bindDocumentClickListener, unbindDocumentClickListener] =
        useEventListener({
            ...
            listener: (event) => {
                if (
                    !isVisible(targetRef.current) ||
                    getComputedStyle(targetRef.current).getPropertyValue(
                        "position",
                    ) === "static"
                ) {
