...
export const TreeSelect = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const onFirstHiddenFocus = (event) => {
            const focusableEl =
                event.relatedTarget === focusInputRef.current
        ...
        const onTabKey = (event, pressedInInputText = false) => {
            if (!pressedInInputText) {
                if (overlayVisibleState && hasFocusableElements()) {
                    DomHandler.focus(
                        firstHiddenFocusableElementOnOverlay.current,
                    );
        ...
        const findSelectedNodes = (node, keys, selectedNodes) => {
            if (node) {
                ...
                for (const childNode of props.options) {
                    findSelectedNodes(childNode, keys, selectedNodes);
                }
            }
        };
        ...
        const isSelected = (node, keys) => {
            return isCheckboxSelectionMode
                ? keys[node.key] && keys[node.key].checked
                : keys[node.key];
        };

        const updateTreeState = () => {
            const keys = isSingleSelectionMode
                ? { [`${props.value}`]: true }
                : { ...props.value };
        ...
        const setFocusToFocusableFirstNode = () => {
            const treeNodeEl = DomHandler.find(
                treeRef.current.getElement(),
        ...
        const expandPath = (path) => {
            if (path.length > 0) {
                const expandedKeys = { ...(expandedKeysState || {}) };
        ...
        const createFilterElement = () => {
            if (props.filter) {
                const filterValue = ObjectUtils.isNotEmpty(filteredValue)
                    ? filteredValue
                    : "";
                ...
                const filterProps = mergeProps(
                    {
                        ...
                        onKeyDown: (event) =>
                            onHeaderElementKeyDown(event, false),
                ...
                if (props.filterTemplate) {
                    const defaultContentOptions = {
                        ...
                        filterInputKeyDown: (event) =>
                            onHeaderElementKeyDown(event, () => {}),
                        filterInputChange: onFilterInputChange,
                        filterIconClassName: "p-dropdown-filter-icon",
                        props,
                    };

                    filterContent = ObjectUtils.getJSXElement(
