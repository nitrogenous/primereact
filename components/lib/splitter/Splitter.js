...
export const Splitter = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const panelSize = (sizes, index) =>
			index in sizes
				? sizes[index]
				: (props.children && [].concat(props.children)[index].props.size) ||
				  100 / childrenLength;
		...
		const validateResize = (newPrevPanelSize, newNextPanelSize) => {
			...
			if (
				props.children[prevPanelIndex.current].props &&
				props.children[prevPanelIndex.current].props.minSize &&
				props.children[prevPanelIndex.current].props.minSize > newPrevPanelSize
			) {
				return false;
			}

			if (
				props.children[prevPanelIndex.current + 1].props &&
				props.children[prevPanelIndex.current + 1].props.minSize &&
				props.children[prevPanelIndex.current + 1].props.minSize >
					newNextPanelSize
			) {
				return false;
		...
		const onResizeStart = (event, index, isKeyDown) => {
			...
			if (isKeyDown) {
				prevPanelSize.current = horizontal
					? DomHandler.getOuterWidth(prevPanelElement.current, true)
					: DomHandler.getOuterHeight(prevPanelElement.current, true);
				nextPanelSize.current = horizontal
					? DomHandler.getOuterWidth(nextPanelElement.current, true)
					: DomHandler.getOuterHeight(nextPanelElement.current, true);
			} else {
		...
		const onResize = (event, step = 0, isKeyDown = false) => {
			...
			if (isKeyDown) {
				if (horizontal) {
					newPrevPanelSize =
						(100 * (prevPanelSize.current + step)) / size.current;
					newNextPanelSize =
						(100 * (nextPanelSize.current - step)) / size.current;
				} else {
					newPrevPanelSize =
						(100 * (prevPanelSize.current - step)) / size.current;
					newNextPanelSize =
		...
		const onResizeEnd = (event) => {
			...
			gutterRefs.current &&
				Object.keys(gutterRefs.current).forEach((key) =>
					gutterRefs.current[key].setAttribute(
						"data-p-splitter-gutter-resizing",
						false,
					),
				);
			!isUnstyled() &&
		...
		React.useEffect(() => {
			const panelElements = [...elementRef.current.children].filter(
				(child) =>
					DomHandler.getAttribute(child, "data-pc-section") ===
					"splitterpanel.root",
			);
			...
			panelElements.map((panelElement, i) => {
				...
				if (
					panelElement.childNodes &&
					ObjectUtils.isNotEmpty(
						DomHandler.find(panelElement, "[data-pc-name='splitter']") &&
							DomHandler.find(panelElement, "[data-pc-section='root']"),
					)
				) {
					!isUnstyled() &&
