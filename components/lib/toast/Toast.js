...
export const Toast = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const replace = (messageInfo) => {
			setMessagesState((previousMessagesState) =>
				assignIdentifiers(previousMessagesState, messageInfo, false),
			);
		};

		const remove = (messageInfo) => {
			// allow removal by ID or by message equality
			const removeMessage = messageInfo._pId
				? messageInfo.message
				: messageInfo;
			...
			setMessagesState((prev) =>
				prev.filter(
					(msg) =>
						msg._pId !== messageInfo._pId &&
						!ObjectUtils.deepEquals(msg.message, removeMessage),
				),
			);
						...
						{messagesState &&
							messagesState.map((messageInfo, index) => {
								...
								return (
									...
									>
										{inProps.content ? (
											ObjectUtils.getJSXElement(inProps.content, {
