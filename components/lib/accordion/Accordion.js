...
export const Accordion = React.forwardRef((inProps, ref) => {
	...
	const findNextHeaderAction = (tabElement, selfCheck = false) => {
		const nextTabElement = selfCheck
			? tabElement
		...
		return headerElement
			? DomHandler.getAttribute(headerElement, "data-p-disabled")
				? findNextHeaderAction(headerElement.parentElement)
	...
	const findPrevHeaderAction = (tabElement, selfCheck = false) => {
		const prevTabElement = selfCheck
			? tabElement
		...
		return headerElement
			? DomHandler.getAttribute(headerElement, "data-p-disabled")
				? findPrevHeaderAction(headerElement.parentElement)
				: DomHandler.findSingle(
						headerElement,
						'[data-pc-section="headeraction"]',
				  )
	...
	const createTabHeader = (tab, selected, index) => {
		...
		const header = getTabProp(tab, "headerTemplate") ? (
			ObjectUtils.getJSXElement(getTabProp(tab, "headerTemplate"), tabCProps)
		) : (
			<span {...headerTitleProps}>
				{ObjectUtils.getJSXElement(getTabProp(tab, "header"), tabCProps)}
			</span>
		);
		...
		const icon = selected
			? props.collapseIcon || <ChevronDownIcon {...headerIconProps} />
			: props.expandIcon || <ChevronRightIcon {...headerIconProps} />;
		const toggleIcon = IconUtils.getJSXIcon(
			icon,
			{ ...headerIconProps },
			{ props, selected },
		);
		const headerProps = mergeProps(
			{
				className: classNames(
					...
					cx("tab.header", { selected, getTabProp, tab }),
				),
				style,
		...
		const toggleableContentProps = mergeProps(
			{
				...
				ref: contentRef,
				className: classNames(
					getTabProp(tab, "contentClassName"),
					getTabProp(tab, "className"),
					cx("tab.toggleablecontent"),
				),
				style,
				role: "region",
