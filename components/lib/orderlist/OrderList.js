...
export const OrderList = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const getVisibleList = () => {
            if (hasFilter) {
                const filterValue = filterValueState
                    .trim()
        ...
        const onItemClick = (event) => {
            ...
            const selectedIndex = ObjectUtils.findIndexInList(
                value,
                selectionState,
            );
            const listElement = getListElement();
            const selectedId = DomHandler.find(
                listElement,
            ...
            if (selected)
                newSelection = metaKey
                    ? selectionState.filter((_, i) => i !== selectedIndex)
                    : [value];
        ...
        const setSelectionStateWithIndex = (index) => {
            ...
            const selected =
                ObjectUtils.findIndexInList(item, selectionState) !== -1;

            if (selected)
                setSelectionState(
                    selectionState.filter(
        ...
        const onHomeKey = (event) => {
            if (event.ctrlKey && event.shiftKey) {
                ...
                setSelectionState(
                    [...visibleList].slice(0, matchedOptionIndex + 1),
                );
            } else {
        ...
        const onEndKey = (event) => {
            ...
            if (event.ctrlKey && event.shiftKey) {
                ...
                setSelectionState(
                    [...visibleList].slice(matchedOptionIndex, items.length),
                );
            } else {
                changeFocusedOptionIndex(
                    DomHandler.find(listElement, '[data-pc-section="item"]')
                        .length - 1,
                );
            }
        ...
        const changeFocusedOptionIndex = (index) => {
            ...
            const _focusedOptionIndex = items[order]
                ? items[order].getAttribute("id")
                : -1;
        ...
        const onFilter = (event) => {
            const _filterValue = event.target.value;
