...
export const OrderListSubList = React.memo(
    React.forwardRef((props, ref) => {
        ...
        const onDragOver = (event, index) => {
            if (
                draggedItemIndex.current !== index &&
                draggedItemIndex.current + 1 !== index
            ) {
                ...
                event.target.setAttribute(
                    "data-p-orderlist-droppoint-highlight",
                    true,
                );
        ...
        const onListMouseMove = (event) => {
            if (dragging.current) {
                const offsetY =
                    listElementRef.current.getBoundingClientRect().top +
                    DomHandler.getWindowScrollTop();
                const bottomDiff =
                    offsetY + listElementRef.current.clientHeight - event.pageY;
            ...
            const listProps = mergeProps(
                {
                    ...
                    "aria-activedescendant": props.focused
            ...
            if (props.filter) {
                ...
                if (props.filterTemplate) {
                    const defaultContentOptions = {
                        ...
                        inputProps: {
                            inputClassName:
                                "p-orderlist-filter-input p-inputtext p-component",
                            onChange: props.onFilter,
                            onKeyDown: onFilterInputKeyDown,
                        },
