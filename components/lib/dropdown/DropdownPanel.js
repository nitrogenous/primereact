...
export const DropdownPanel = React.memo(
	React.forwardRef((props, ref) => {
		...
		const isEmptyFilter =
			!(props.visibleOptions && props.visibleOptions.length) && props.hasFilter;
		const filterOptions = {
			...
			props.onEnter(() => {
				if (props.virtualScrollerRef.current) {
					...
					if (selectedIndex !== -1) {
						setTimeout(
							() =>
								props.virtualScrollerRef.current.scrollToIndex(selectedIndex),
							0,
						);
					}
				}
				...
				const clearIconProps = mergeProps(
					{
						...
						onClick: () =>
							props.onFilterClearIconClick(() =>
								DomHandler.focus(filterInputRef.current),
							),
					},
				...
				const virtualScrollerProps = {
					...
					...{
						...
						contentTemplate: (options) => {
							const emptyMessage = props.hasFilter
