	...
	React.forwardRef((inProps, ref) => {
		...
		const getVisibleOptions = () => {
			if (hasFilter && !isLazy) {
				...
				const searchFields = props.filterBy
					? props.filterBy.split(",")
				...
				if (props.optionGroupLabel) {
					...
					for (const optgroup of props.options) {
						...
						if (filteredSubOptions && filteredSubOptions.length) {
							filteredGroups.push({
								...optgroup,
								...{ [`${props.optionGroupChildren}`]: filteredSubOptions },
							});
						}
					}
		...
		const findNextOption = (index) => {
			if (props.optionGroupLabel) {
				...
				const option = findNextOptionInList(
					getOptionGroupChildren(visibleOptions[groupIndex]),
					optionIndex,
				);
		...
		const findPrevOption = (index) => {
			...
			if (props.optionGroupLabel) {
				...
				const option = findPrevOptionInList(
					getOptionGroupChildren(visibleOptions[groupIndex]),
					optionIndex,
				);
				...
				if (option) return option;
					...
					return findPrevOption({
						group: groupIndex - 1,
						option: getOptionGroupChildren(visibleOptions[groupIndex - 1])
							.length,
					});
				else return null;
			}
		...
		const search = (event) => {
			...
			if (currentSearchChar.current === char) searchValue.current = char;
				...
				searchValue.current = searchValue.current
		...
		const searchOption = (index) => {
			if (searchValue.current) {
				return (
					searchOptionInRange(index, visibleOptions.length) ||
					searchOptionInRange(0, index)
				);
			}
		...
		const searchOptionInGroup = (index) => {
			...
			for (let i = searchIndex.group; i < visibleOptions.length; i++) {
				const groupOptions = getOptionGroupChildren(visibleOptions[i]);
		...
		const matchesSearchValue = (option) => {
			...
			return label.startsWith(
				searchValue.current.toLocaleLowerCase(props.filterLocale),
			);
		};
			...
			if (props.value != null && options) {
				if (props.optionGroupLabel) {
					for (let i = 0; i < options.length; i++) {
						const selectedOptionIndex = findOptionIndexInList(
							props.value,
							getOptionGroupChildren(options[i]),
						);
		...
		const findOptionIndexInList = (value, list) => {
			...
			return list.findIndex((item) =>
				ObjectUtils.equals(value, getOptionValue(item), key),
			);
		};
		...
		const scrollInView = () => {
			const highlightItem = DomHandler.findSingle(
				overlayRef.current,
		...
		const getOptionRenderKey = (option) => {
			return props.dataKey
				? ObjectUtils.resolveFieldData(option, props.dataKey)
				: getOptionLabel(option);
		};

		const isOptionDisabled = (option) => {
			if (props.optionDisabled) {
				return ObjectUtils.isFunction(props.optionDisabled)
					? props.optionDisabled(option)
					: ObjectUtils.resolveFieldData(option, props.optionDisabled);
			}
		...
		const getSelectedOption = () => {
			...
			return index !== -1
				? props.optionGroupLabel
					? getOptionGroupChildren(props.options[index.group])[index.option]
					: props.options[index]
				: null;
		};
				...
				option = {
					value:
						typeof optionValue === "object"
							? props.options.findIndex((o) => o === optionValue)
							: optionValue,
		...
		const createLabel = () => {
			const label = ObjectUtils.isNotEmpty(selectedOption)
		...
		const createDropdownIcon = () => {
			...
			const icon = props.dropdownIcon || (
				<ChevronDownIcon {...dropdownIconProps} />
			);
			const dropdownIcon = IconUtils.getJSXIcon(
				icon,
				{ ...dropdownIconProps },
				{ props },
			);
		...
		const dropdownIcon = props.loading
			? createLoadingIcon()
			: createDropdownIcon();
		...
		const rootProps = mergeProps(
			{
				...
				className: classNames(
					props.className,
					cx("root", { focusedState, overlayVisibleState }),
				),
				style: props.style,
				onClick: (e) => onClick(e),
