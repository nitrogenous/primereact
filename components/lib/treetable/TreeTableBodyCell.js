...
export const TreeTableBodyCell = (props) => {
    ...
    const getColumnPTOptions = (key) => {
        const isSingleSelectionMode =
            props.metaData.props.selectionMode === "single";
        const isMultipleSelectionMode =
            props.metaData.props.selectionMode === "multiple";
    ...
    const switchCellToViewMode = (event) => {
        if (props.cellEditValidator) {
            const valid = props.cellEditValidator({
                originalEvent: event,
                columnProps: props,
            });
    ...
    React.useEffect(() => {
        if (elementRef.current && props.editor) {
            ...
            if (editingState) {
                const focusable = DomHandler.findSingle(
                    elementRef.current,
                    "input",
                );
    ...
    const bodyClassName = ObjectUtils.getPropValue(
        props.bodyClassName,
        props.node.data,
        { field: props.field, rowIndex: props.rowIndex, props: props },
    );
    ...
    if (editingState) {
        if (props.editor)
            content = ObjectUtils.getJSXElement(props.editor, {
                node: props.node,
                rowData: props.rowData,
                value: ObjectUtils.resolveFieldData(
                    props.node.data,
                    props.field,
                ),
                field: props.field,
    ...
    const bodyCellProps = mergeProps(
        {
            ...
            className: classNames(
                ...
                cx("bodyCell", { bodyProps: props, editingState, align }),
            ),
            style,
            onClick: (e) => onClick(e),
