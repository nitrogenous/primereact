...
export const TreeTableScrollableView = React.memo((props) => {
	...
	const setScrollHeight = () => {
		if (props.scrollHeight) {
			if (props.scrollHeight.indexOf("%") !== -1) {
				const datatableContainer = findDataTableContainer(elementRef.current);
				...
				const containerHeight = DomHandler.getOuterHeight(datatableContainer);
				const relativeHeight =
					(DomHandler.getOuterHeight(datatableContainer.parentElement) *
						parseInt(props.scrollHeight, 10)) /
					100;
	...
	const findDataTableContainer = (element) => {
		if (element) {
			...
			while (
				el &&
				!(
					DomHandler.getAttribute(el, "data-pc-section") === "root" ||
					DomHandler.getAttribute(el, "data-pc-name") === "treetable"
				)
			) {
				el = el.parentElement;
			}
	...
	const onBodyScroll = () => {
		const frozenView = elementRef.current.previousElementSibling;
		let frozenScrollBody;

		if (frozenView) {
			frozenScrollBody = DomHandler.findSingle(
				frozenView,
				'[data-pc-section="scrollablebody"]',
			);
		}
	...
	useMountEffect(() => {
		let el = DomHandler.find(
			findDataTableContainer(elementRef.current),
			'[data-pc-section="scrollablebody"]',
		);
	...
	const scrollableBodyProps = mergeProps(
		{
			...
			style:
				!props.frozen && props.scrollHeight
					? { overflowY: "scroll" }
					: undefined,
