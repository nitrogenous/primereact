...
export const TreeTableFooter = React.memo((props) => {
	...
	const createFooterCell = (column, index) => {
		const footerCellProps = mergeProps(
			{
				...
				className:
					getColumnProp(column, "footerClassName") ||
					getColumnProp(column, "className"),
				style:
					getColumnProp(column, "footerStyle") ||
		...
		const content = ObjectUtils.getJSXElement(getColumnProp(column, "footer"), {
	...
	const createFooterRow = (row, index) => {
		...
		const footerRowProps = mergeProps(
			ptm("footerRow", { hostName: props.hostName, role: "row" }),
		);
