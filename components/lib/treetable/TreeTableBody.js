...
export const TreeTableBody = React.memo((props) => {
    ...
    const onRowClick = (event, node) => {
        ...
        if (
            (isSingleSelectionMode || isMultipleSelectionMode) &&
            ...
            if (isMultipleSelectionMode && event.shiftKey) {
                ...
                const anchorRowIndex = flatKeys.findIndex(
                    (key) => props.selectionKeys[key],
                );
    ...
    const createRows = () => {
        if (props.paginator && !props.lazy) {
            const rpp = props.rows || 0;
            const startIndex = props.first || 0;
            const endIndex = startIndex + rpp;
            const rows = [];

            for (let i = startIndex; i < endIndex; i++) {
                const rowData = props.value[i];
