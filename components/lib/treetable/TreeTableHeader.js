...
export const TreeTableHeader = React.memo((props) => {
	...
	const getColumnProp = (column, ...args) => {
		return column
			? typeof args[0] === "string"
				? ColumnBase.getCProp(column, args[0])
				: ColumnBase.getCProp(args[0] || column, args[1])
			: null;
	};
	...
	const onHeaderClick = (event, column) => {
		if (getColumnProp(column, "sortable")) {
			...
			if (
				DomHandler.getAttribute(targetNode, "data-p-sortable-column") ===
					true ||
				DomHandler.getAttribute(targetNode, "data-pc-section") ===
					"headertitle" ||
				DomHandler.getAttribute(targetNode, "data-pc-section") === "sorticon" ||
				DomHandler.getAttribute(targetNode.parentElement, "data-pc-section") ===
					"sorticon" ||
	...
	const createSortIcon = (column, sorted, sortOrder) => {
		if (getColumnProp(column, "sortable")) {
			...
			const sortIcon = IconUtils.getJSXIcon(
				props.sortIcon || icon,
				{ ...sortIconProps },
				{ props, sorted, sortOrder },
			);
	...
	const createTitle = (column, options) => {
		const title = ObjectUtils.getJSXElement(getColumnProp(column, "header"), {
			props: options,
		});
		...
		if (getColumnProp(column, "filter") && options.renderFilter) {
			filterElement = getColumnProp(column, "filterElement") || (
				<InputText
					...
					defaultValue={
						props.filters && props.filters[getColumnProp(column, "field")]
							? props.filters[getColumnProp(column, "field")].value
							: null
					}
			...
			const headerCellProps = mergeProps(
				{
					...
					className: classNames(
						cx("headerCell", { options, frozen }),
						getColumnProp(column, "filterHeaderClassName"),
					),
					style:
						getColumnProp(column, "filterHeaderStyle") ||
			...
			const headerCellProps = mergeProps(
				{
					role: "columnheader",
					className: classNames(
						getColumnProp(column, "headerClassName") ||
							getColumnProp(column, "className"),
						cx("headerCell", {
							headerProps: props,
							frozen,
							column,
	...
	const createHeaderRow = (row, index) => {
		const rowColumns = React.Children.toArray(
			RowBase.getCProp(row, "children"),
		);
		const rowHeaderCells = rowColumns.map((col, i) =>
	...
	const createColumnGroup = () => {
		const rows = React.Children.toArray(
			ColumnGroupBase.getCProp(props.columnGroup, "children"),
		);
	...
	const createColumns = (columns) => {
		if (columns) {
			const headerRowProps = mergeProps(
				ptm("headerRow", { hostName: props.hostName, role: "row" }),
			);
