...
export const TreeTableHeader = React.memo((props) => {
        ...
        if (getColumnProp(column, "filter") && props.onFilter) {
            ...
            filterTimeout.current = setTimeout(() => {
                props.onFilter({
                    ...
                    matchMode:
                        getColumnProp(column, "filterMatchMode") ||
                        "startsWith",
                });
    ...
    const createTitle = (column, options) => {
        const title = ObjectUtils.getJSXElement(
        ...
        if (options.filterOnly) {
            ...
            const multiSortMetaData =
                sortMetaDataIndex !== -1
                    ...
                    : null;
            const singleSorted =
                getColumnProp(column, "field") === props.sortField;
            const multipleSorted = multiSortMetaData !== null;
            const sorted =
                getColumnProp(column, "sortable") &&
