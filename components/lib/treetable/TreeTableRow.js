...
export const TreeTableRow = React.memo((props) => {
	...
	const expand = (event, navigateFocusToChild = false) => {
		const expandedKeys = props.expandedKeys ? { ...props.expandedKeys } : {};
	...
	const onArrowDownKey = (event) => {
		...
		nextElementSibling &&
			focusRowChange(event.currentTarget, nextElementSibling);
	...
	const onArrowUpKey = (event) => {
		...
		previousElementSibling &&
			focusRowChange(event.currentTarget, previousElementSibling);
	...
	const onArrowRightKey = (event) => {
		const ishiddenIcon =
			DomHandler.findSingle(event.currentTarget, "button").style.visibility ===
			"hidden";
		const togglerElement = DomHandler.findSingle(
			elementRef.current,
			'[data-pc-section="rowtoggler"]',
		);
	...
	const onArrowLeftKey = (event) => {
		...
		const ishiddenIcon =
			DomHandler.findSingle(currentTarget, "button").style.visibility ===
			"hidden";
		const togglerElement = DomHandler.findSingle(
			currentTarget,
			'[data-pc-section="rowtoggler"]',
	...
	const onHomeKey = (event) => {
		const findFirstElement = DomHandler.findSingle(
	...
	const onTabKey = () => {
		...
		const hasSelectedRow = rows.some(
			(row) =>
				DomHandler.getAttribute(row, "data-p-highlight") ||
	...
	const setTabIndexForSelectionMode = (event, nodeTouched) => {
		if (props.selectionMode !== null) {
			const elements = [
				...DomHandler.find(elementRef.current.parentElement, "tr"),
			];
	...
	const createCheckbox = (column) => {
		if (props.selectionMode === "checkbox" && props.node.selectable !== false) {
			...
			const icon = checked
				? props.checkboxIcon || <CheckIcon {...checboxIconProps} />
				: partialChecked
				  ? props.checkboxIcon || <MinusIcon {...checboxIconProps} />
				  : null;
			const checkIcon = IconUtils.getJSXIcon(
				icon,
				{ ...checboxIconProps },
				{ props, checked, partialChecked },
			);
	...
	const createCell = (column, index) => {
		...
		return (
			<TreeTableBodyCell
				hostName={props.hostName}
				key={`${
					getColumnProp(column, "columnKey") || getColumnProp(column, "field")
				}_${index}`}
	...
	if (props.rowClassName) {
		const rowClassName = props.rowClassName(props.node);
