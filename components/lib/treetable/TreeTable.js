...
export const TreeTable = React.forwardRef((inProps, ref) => {
	...
	const sortNodes = (data) => {
		...
		if (columnSortable.current && columnSortFunction.current) {
			...
			for (const node of data) {
				lookupMap.set(
					node.data,
					ObjectUtils.resolveFieldData(node.data, sortField),
				);
			}
	...
	const multisortField = (node1, node2, multiSortMeta, index, comparator) => {
		...
		if (ObjectUtils.compare(value1, value2, comparator) === 0) {
			return multiSortMeta.length - 1 > index
				? multisortField(node1, node2, multiSortMeta, index + 1, comparator)
				: 0;
		}
	...
	const onColumnResizeStart = (event) => {
		...
		lastResizerHelperX.current =
			event.originalEvent.pageX - containerLeft + elementRef.current.scrollLeft;
	...
	const onColumnResizeEnd = (event) => {
		const delta =
			resizerHelperRef.current.offsetLeft - lastResizerHelperX.current;
		const columnWidth = resizeColumn.current.offsetWidth;
		const newColumnWidth = columnWidth + delta;
		const minWidth = resizeColumn.current.style.minWidth || 15;

		if (columnWidth + delta > parseInt(minWidth, 10)) {
			if (props.columnResizeMode === "fit") {
				const nextColumn = resizeColumn.current.nextElementSibling;
				const nextColumnWidth = nextColumn.offsetWidth - delta;

				if (newColumnWidth > 15 && nextColumnWidth > 15) {
					if (props.scrollable) {
						const scrollableView = findParentScrollableView(resizeColumn.current);
						const scrollableBodyTable = DomHandler.findSingle(
							scrollableView,
							'table[data-pc-section="scrollablebodytable"]',
						);
						const scrollableHeaderTable = DomHandler.findSingle(
							scrollableView,
							'table[data-pc-section="scrollableheadertable"]',
						);
						const scrollableFooterTable = DomHandler.findSingle(
							scrollableView,
							'table[data-pc-section="scrollablefootertable"]',
						);
						const resizeColumnIndex = DomHandler.index(resizeColumn.current);
	...
	const findParentScrollableView = (column) => {
		if (column) {
			...
			while (
				parent &&
				DomHandler.getAttribute(parent, "data-pc-section") !== "scrollable"
			) {
	...
	const onColumnDragStart = (e) => {
		...
		iconWidth.current = DomHandler.getHiddenElementOuterWidth(
			reorderIndicatorUpRef.current,
		);
		iconHeight.current = DomHandler.getHiddenElementOuterHeight(
			reorderIndicatorUpRef.current,
		);
	...
	const onColumnDrop = (e) => {
		...
		if (draggedColumnEl.current) {
			const dragIndex = DomHandler.index(draggedColumnEl.current);
			...
			if (allowDrop) {
				const columns = columnOrderState
					? getColumns()
					: React.Children.toArray(props.children);
				const isSameColumn = (col1, col2) =>
					getColumnProp(col1, "columnKey") || getColumnProp(col2, "columnKey")
						? ObjectUtils.equals(col1, col2, "props.columnKey")
						: ObjectUtils.equals(col1, col2, "props.field");
				...
				for (const column of columns) {
					columnOrder.push(
						getColumnProp(column, "columnKey") ||
	...
	const findColumnByKey = (columns, key) => {
		if (columns && columns.length) {
			for (let i = 0; i < columns.length; i++) {
				const child = columns[i];

				if (
					getColumnProp(child, "columnKey") === key ||
					getColumnProp(child, "field") === key
				) {
					return child;
				}
			}
	...
	const filterLocal = (value) => {
		...
		for (const node of value) {
			...
			for (let j = 0; j < columns.length; j++) {
				const col = columns[j];
				const filterMeta = filters ? filters[getColumnProp(col, "field")] : null;
				const filterField = getColumnProp(col, "field");
				...
				if (filterMeta) {
					...
					filterConstraint =
						filterMatchMode === "custom"
							? getColumnProp(col, "filterFunction")
							: FilterService.filters[filterMatchMode];
					options = {
						rowData: node,
						filters,
						props,
					...
					if (
						(isStrictMode &&
							!(
								findFilteredNodes(copyNode, paramsWithoutNode) ||
								isFilterMatched(copyNode, paramsWithoutNode)
							)) ||
						(!isStrictMode &&
							!(
								isFilterMatched(copyNode, paramsWithoutNode) ||
				...
				if (props.globalFilter && !globalMatch) {
					...
					if (
						(isStrictMode &&
							(findFilteredNodes(copyNodeForGlobal, paramsWithoutNode) ||
								isFilterMatched(copyNodeForGlobal, paramsWithoutNode))) ||
						(!isStrictMode &&
							(isFilterMatched(copyNodeForGlobal, paramsWithoutNode) ||
								findFilteredNodes(copyNodeForGlobal, paramsWithoutNode)))
					) {
	...
	const isFilterMatched = (
		node,
		{ filterField, filterValue, filterConstraint, isStrictMode, options },
	) => {
		let matched = false;
		const dataFieldValue = ObjectUtils.resolveFieldData(node.data, filterField);

		if (
			filterConstraint(dataFieldValue, filterValue, props.filterLocale, options)
		) {
			matched = true;
		}
	...
	const createScrollableTable = (value) => {
		...
		const scrollableColumns = frozenColumns
			? getScrollableColumns(columns)
			: columns;
		let frozenView, scrollableView;

		if (frozenColumns) {
			frozenView = createScrollableView(
				value,
				frozenColumns,
				true,
				props.frozenHeaderColumnGroup,
				props.frozenFooterColumnGroup,
			);
		}
		...
		scrollableView = createScrollableView(
			...
			scrollableColumns,
			false,
			props.headerColumnGroup,
			props.footerColumnGroup,
		);
	...
	const createTable = (value) => {
		return props.scrollable
			? createScrollableTable(value)
			: createRegularTable(value);
	};
	...
	const headerFacet = props.header && (
		<div {...headerProps}>{props.header}</div>
	);
	const footerFacet = props.footer && (
		<div {...footerProps}>{props.footer}</div>
	);
	const paginatorTop =
		props.paginator &&
		props.paginatorPosition !== "bottom" &&
		createPaginator("top", totalRecords);
	const paginatorBottom =
		props.paginator &&
		props.paginatorPosition !== "top" &&
		createPaginator("bottom", totalRecords);
	const loader = createLoader();
	const resizeHelper = props.resizableColumns && (
		<div ref={resizerHelperRef} {...resizeHelperProps}></div>
	);
	...
	const reorderIndicatorDownProps = {
	...
	const rootProps = mergeProps(
		{
			...
			className: classNames(
				props.className,
				ptCallbacks.cx("root", { isRowSelectionMode }),
			),
			style: props.style,
