...
export const TreeTable = React.forwardRef((inProps, ref) => {
    ...
    const sortMultipleNodes = (data, multiSortMeta) => {
        ...
        for (let i = 0; i < value.length; i++) {
            if (value[i].children && value[i].children.length) {
                value[i].children = sortMultipleNodes(
                    value[i].children,
                    multiSortMeta,
                );
            }
        ...
        if (columnWidth + delta > parseInt(minWidth, 10)) {
            if (props.columnResizeMode === "fit") {
                ...
                if (newColumnWidth > 15 && nextColumnWidth > 15) {
                    if (props.scrollable) {
                        const scrollableView = findParentScrollableView(
                            resizeColumn.current,
                        );
    ...
    const findParentScrollableView = (column) => {
        if (column) {
            ...
            while (
                parent &&
                DomHandler.getAttribute(parent, "data-pc-section") !==
                    "scrollable"
            ) {
        ...
        for (const node of value) {
            ...
            for (let j = 0; j < columns.length; j++) {
                ...
                const filterMeta = filters
                    ? filters[getColumnProp(col, "field")]
                    : null;
                ...
                if (props.globalFilter && !globalMatch) {
                    ...
                    filterConstraint =
                        FilterService.filters[props.globalFilterMatchMode];
                    paramsWithoutNode = {
                        filterField,
                        filterValue,
                        filterConstraint,
    ...
    const isFilterMatched = (
        ...
        let matched = false;
        const dataFieldValue = ObjectUtils.resolveFieldData(
            node.data,
            filterField,
        );
