...
export const Tree = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const filteredValue = props.onFilterValueChange
			? props.filterValue
			: filterValueState;
		const expandedKeys = props.onToggle
			? props.expandedKeys
			: expandedKeysState;
		...
		const getRootNode = () => {
			return props.filter && filteredNodes.current
		...
		const onDrop = (event) => {
			if (validateDropNode(dragState.current.path, event.path)) {
				...
				const dragPaths = dragState.current.path.split("-");

				dragPaths.pop();

				const dragNodeParent = findNode(value, dragPaths);
				const dragNode = dragNodeParent
					? dragNodeParent.children[dragState.current.index]
					: value[dragState.current.index];
				const dropNode = findNode(value, event.path.split("-"));
		...
		const onDropPoint = (event) => {
			if (validateDropPoint(event)) {
				...
				const dropPaths = event.path.split("-");

				dropPaths.pop();

				const dragNodeParent = findNode(value, dragPaths);
				const dropNodeParent = findNode(value, dropPaths);
				const dragNode = dragNodeParent
					? dragNodeParent.children[dragState.current.index]
					: value[dragState.current.index];
				const siblings = areSiblings(dragState.current.path, event.path);
				...
				if (event.position < 0) {
					const dropIndex = siblings
						? dragState.current.index > event.index
							? event.index
							: event.index - 1
						: event.index;
		...
		const validateDropNode = (dragPath, dropPath) => {
			const _validateDrop = validateDrop(dragPath, dropPath);

			if (_validateDrop) {
				//child dropped on parent
				if (
					dragPath.indexOf("-") > 0 &&
					dragPath.substring(0, dragPath.lastIndexOf("-")) === dropPath
				) {
					return false;
				}
		...
		const validateDropPoint = (event) => {
			const _validateDrop = validateDrop(dragState.current.path, event.path);

			if (_validateDrop) {
				//child dropped to next sibling's drop point
				if (
					event.position === -1 &&
					areSiblings(dragState.current.path, event.path) &&
					dragState.current.index + 1 === event.index
				) {
					return false;
				}
		...
		const isFilterMatched = (
			node,
			{ searchFields, filterText, isStrictMode },
		) => {
			let matched = false;
			...
			if (!matched || (isStrictMode && !isNodeLeaf(node))) {
				matched =
					findFilteredNodes(node, { searchFields, filterText, isStrictMode }) ||
					matched;
			}
		...
		const createEmptyMessageNode = () => {
			...
			return (
				<li {...emptyMessageProps}>
					<span className="p-treenode-content">
		...
		const createFilter = () => {
			if (props.filter) {
				const value = ObjectUtils.isNotEmpty(filteredValue)
					? filteredValue
					: "";
				...
				const filterIcon = IconUtils.getJSXIcon(
					icon,
					{ ...searchIconProps },
					{ props },
				);
