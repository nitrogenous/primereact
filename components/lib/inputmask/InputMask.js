...
export const InputMask = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const isCompleted = () => {
            for (
                let i = firstNonMaskPos.current;
                i <= lastRequiredNonMaskPos.current;
                i++
            ) {
                if (
                    tests.current[i] &&
        ...
        const getValue = () => {
            return props.unmask
                ? getUnmaskedValue()
                : elementRef.current && elementRef.current.value;
        };
        ...
        const shiftL = (begin, end) => {
            ...
            for (i = begin, j = seekNext(end); i < len.current; i++) {
                if (tests.current[i]) {
                    if (
                        j < len.current &&
                        tests.current[i].test(buffer.current[j])
                    ) {
        ...
        const handleAndroidInput = (e) => {
            const curVal = elementRef.current.value;
            const pos = caret();
            ...
            if (
                oldVal.current.length &&
                oldVal.current.length > curVal.length
            ) {
                ...
                if (pos.begin === 0) {
                    while (
                        pos.begin < firstNonMaskPos.current &&
        ...
        const checkVal = (allow) => {
            ...
            if (allow) {
                ...
                if (
                    props.autoClear ||
                    buffer.current.join("") === defaultBuffer.current
                ) {
                ...
                if (elementRef.current) {
                    elementRef.current.value =
                        elementRef.current.value.substring(0, lastMatch + 1);
                }
            }
        ...
        const updateFilledState = () => {
            if (
                elementRef.current &&
                elementRef.current.value &&
                elementRef.current.value.length > 0
            )
        ...
        const init = () => {
            if (props.mask) {
                ...
                androidChrome.current =
                    DomHandler.isChrome() && DomHandler.isAndroid();

                const maskTokens = props.mask.split("");
                ...
                for (let i = 0; i < maskTokens.length; i++) {
                    ...
                    if (c === "?") {
                        ...
                        if (i < partialPosition.current) {
                            lastRequiredNonMaskPos.current =
                                tests.current.length - 1;
                        }
                    } else {
