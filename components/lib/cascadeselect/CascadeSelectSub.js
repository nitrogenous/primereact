...
export const CascadeSelectSub = React.memo((props) => {
    ...
    const position = () => {
        ...
        const sublistWidth = elementRef.current.offsetParent
            ? elementRef.current.offsetWidth
            : DomHandler.getHiddenElementOuterWidth(element);
    ...
    const findNextItem = (item) => {
        ...
        return nextItem
            ? DomHandler.hasClass(nextItem, "p-disabled") ||
              !DomHandler.hasClass(nextItem, "p-cascadeselect-item")
    ...
    const findPrevItem = (item) => {
        ...
        return prevItem
            ? DomHandler.hasClass(prevItem, "p-disabled") ||
              !DomHandler.hasClass(prevItem, "p-cascadeselect-item")
    ...
    const onOptionClick = (event, option) => {
        if (isOptionGroup(option)) {
            setActiveOptionState((prevActiveOption) =>
                prevActiveOption === option ? null : option,
            );
    ...
    const getOptionGroupChildren = (optionGroup) => {
        return ObjectUtils.resolveFieldData(
            optionGroup,
            props.optionGroupChildren[props.level],
        );
    };
    ...
    useMountEffect(() => {
        if (props.selectionPath && props.options && !props.dirty) {
            const activeOption = props.options.find((o) =>
                props.selectionPath.includes(o),
            );
    ...
    const createOption = (option, index) => {
        ...
        const content = props.template ? (
            ObjectUtils.getJSXElement(props.template, getOptionValue(option))
        ) : (
            <span {...textProps}>{getOptionLabelToRender(option)}</span>
        );
        ...
        const icon = props.optionGroupIcon || (
            <AngleRightIcon {...optionGroupIconProps} />
        );
        const optionGroup =
        ...
        const itemProps = mergeProps(
            {
                className: classNames(
                    option.className,
                    cx("item", { option, isGroup, isSelected }),
                ),
                style: option.style,
                role: "none",
