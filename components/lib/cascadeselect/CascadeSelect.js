...
export const CascadeSelect = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const cascadeSelectOverlayDisplayOrder = useDisplayOrder(
			"cascade-select",
			overlayVisibleState,
		);
		...
		useGlobalOnEscapeKey({
			...
			priority: [
				ESC_KEY_HANDLING_PRIORITIES.CASCADE_SELECT,
				cascadeSelectOverlayDisplayOrder,
			],
		});
		...
		const getOptionGroupChildren = (optionGroup, level) => {
			return ObjectUtils.resolveFieldData(
				optionGroup,
				props.optionGroupChildren[level],
			);
		};
		...
		const findModelOptionInGroup = (option, level) => {
			if (isOptionGroup(option, level)) {
				...
				for (const childOption of getOptionGroupChildren(option, level)) {
				...
				ObjectUtils.equals(props.value, getOptionValue(option), props.dataKey)
			) {
				return [option];
			}
...
`;
		...
		const createLabel = () => {
			const label = props.value
				...
				: props.placeholder || "p-emptylabel";
