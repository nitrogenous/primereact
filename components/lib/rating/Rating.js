...
export const Rating = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const createIcons = () => {
            return Array.from({ length: props.stars }, (_, i) => i + 1).map(
                (value) => {
                    ...
                    const icon = active
                        ? {
                              type: props.onIcon || (
                                  <StarFillIcon {...onIconProps} />
                              ),
                          }
                        : {
                              type: props.offIcon || (
                                  <StarIcon {...offIconProps} />
                              ),
                          };
                    const content = IconUtils.getJSXIcon(
                        icon.type,
                        active ? { ...onIconProps } : { ...offIconProps },
                        { props },
                    );
        ...
        const createCancelIcon = () => {
            if (props.cancel) {
                ...
                const icon = props.cancelIcon || (
                    <BanIcon {...cancelIconProps} />
                );
                const content = IconUtils.getJSXIcon(
                    icon,
                    { ...cancelIconProps, ...props.cancelIconProps },
                    { props },
                );
