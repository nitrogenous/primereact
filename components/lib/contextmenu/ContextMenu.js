...
`;
        ...
        const position = (event) => {
            if (event) {
                ...
                const width = menuRef.current.offsetParent
                    ? menuRef.current.offsetWidth
                    : DomHandler.getHiddenElementOuterWidth(menuRef.current);
                const height = menuRef.current.offsetParent
                    ? menuRef.current.offsetHeight
                    : DomHandler.getHiddenElementOuterHeight(menuRef.current);
        ...
        const isOutsideClicked = (event) => {
            return (
                menuRef?.current &&
                !(
                    menuRef.current.isSameNode(event.target) ||
