...
export const VirtualScroller = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const [numToleratedItemsState, setNumToleratedItemsState] =
            React.useState(props.numToleratedItems);
                ...
                if (isToStart) {
                    if (both) {
                        if (viewport.first.rows - first.rows > index[0]) {
                            ...
                            viewport.first.cols - first.cols >
                            index[1]
                        ) {
                            scrollToItem(
        ...
        const getRenderedRange = () => {
            ...
            if (elementRef.current) {
                ...
                if (both) {
                    firstInViewport = {
                        rows: calculateFirstInViewport(
                            scrollTop,
                            props.itemSize[0],
                        ),
                        cols: calculateFirstInViewport(
                            scrollLeft,
                            props.itemSize[1],
                        ),
                    };
                    lastInViewport = {
                        rows:
                            firstInViewport.rows + numItemsInViewportState.rows,
                        cols:
                            firstInViewport.cols + numItemsInViewportState.cols,
                    };
                } else {
        ...
        const calculateOptions = () => {
            const { numItemsInViewport, numToleratedItems } =
                calculateNumItems();
        ...
        const setContentPosition = (pos) => {
            if (contentRef.current && !props.appendOnly) {
                ...
                if (both) {
                    ...
                    const translateVal = calculateTranslateVal(
                        first,
                        props.itemSize,
                    );
        ...
        const onScrollPositionChange = (event) => {
            ...
            if (both) {
                ...
                if (
                    ...
                    const currentIndex = {
                        rows: calculateCurrentIndex(
                            scrollTop,
                            props.itemSize[0],
                        ),
                        cols: calculateCurrentIndex(
                            scrollLeft,
                            props.itemSize[1],
                        ),
                    };
                ...
                if (
                    !props.appendOnly ||
                    (props.appendOnly && isScrollDownOrRight)
                ) {
            ...
            if (isRangeChanged) {
                ...
                if (props.lazy && isPageChanged(first)) {
                    const newLazyLoadState = {
                        ...
                        last: Math.min(
                            props.step
                                ? (getPageByFirst(first) + 1) * props.step
                                : last,
                            props.items.length,
                        ),
                    };
                    ...
                    const isLazyStateChanged =
                        !lazyLoadState.current ||
                        lazyLoadState.current.first !==
                            newLazyLoadState.first ||
        ...
        const loadedItems = () => {
            ...
            if (items && !loadingState) {
                if (both)
                    return items
                        .slice(
                            props.appendOnly ? 0 : firstState.rows,
                            lastState.rows,
                        )
                        .map((item) =>
                            props.columns
        ...
        const viewInit = () => {
            if (
                elementRef.current &&
                DomHandler.isVisible(elementRef.current)
            ) {
                ...
                defaultHeight.current = DomHandler.getHeight(
                    elementRef.current,
                );
                defaultContentWidth.current = DomHandler.getWidth(
                    contentRef.current,
