...
export const VirtualScroller = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [numItemsInViewportState, setNumItemsInViewportState] =
			React.useState(both ? { rows: 0, cols: 0 } : 0);
		const [numToleratedItemsState, setNumToleratedItemsState] = React.useState(
		...
		const getPageByFirst = (first) => {
			return Math.floor(
				(first + numToleratedItemsState * 4) / (props.step || 1),
			);
		};
		...
		const setContentElement = (element) => {
			contentRef.current =
				element ||
				contentRef.current ||
				DomHandler.findSingle(elementRef.current, ".p-virtualscroller-content");
		};
		...
		const scrollToIndex = (index, behavior = "auto") => {
			...
			if (both) {
				...
				scrollToItem(
					calculateCoord(newFirst.cols, props.itemSize[1], contentPos.left),
					calculateCoord(newFirst.rows, props.itemSize[0], contentPos.top),
				);
				isRangeChanged =
					firstState.rows !== newFirst.rows ||
					firstState.cols !== newFirst.cols;
			} else {
				newFirst = calculateFirst(index, numToleratedItems);
				horizontal
					? scrollToItem(
							calculateCoord(newFirst, props.itemSize, contentPos.left),
							0,
					  )
					: scrollToItem(
							0,
							calculateCoord(newFirst, props.itemSize, contentPos.top),
					  );
				...
				if (isToStart) {
					if (both) {
						if (viewport.first.rows - first.rows > index[0]) {
							scrollToItem(
								viewport.first.cols * props.itemSize[1],
								(viewport.first.rows - 1) * props.itemSize[0],
							);
						} else if (viewport.first.cols - first.cols > index[1]) {
							scrollToItem(
								(viewport.first.cols - 1) * props.itemSize[1],
								viewport.first.rows * props.itemSize[0],
							);
						}
					} else {
			...
			if ((props.columns && both) || horizontal) {
				return loadingState && props.loaderDisabled
					? both
						...
						: loaderArrState
					: props.columns.slice(
							both ? firstState.cols : firstState,
							both ? lastState.cols : lastState,
					  );
			}
		...
		const getRenderedRange = () => {
			const calculateFirstInViewport = (_pos, _size) =>
		...
		const calculateNumItems = () => {
			...
			const calculateNumToleratedItems = (_numItems) =>
		...
		const calculateOptions = () => {
			...
			const last = both
				? {
						rows: calculateLast(
							firstState.rows,
							numItemsInViewport.rows,
							numToleratedItems[0],
						),
						cols: calculateLast(
							firstState.cols,
							numItemsInViewport.cols,
							numToleratedItems[1],
							true,
						),
				  }
			...
			if (props.showLoader) {
				setLoaderArrState(
					both
						? Array.from({ length: numItemsInViewport.rows }).map(() =>
								Array.from({ length: numItemsInViewport.cols }),
						  )
			...
			if (props.autoSize && !loading) {
				Promise.resolve().then(() => {
					if (contentRef.current) {
						...
						const [width, height] = [
							DomHandler.getWidth(elementRef.current),
							DomHandler.getHeight(elementRef.current),
						];
						...
						(both || horizontal) &&
							(elementRef.current.style.width =
								(width < defaultWidth.current
									? width
									: props.scrollWidth || defaultWidth.current) + "px");
						(both || vertical) &&
							(elementRef.current.style.height =
								(height < defaultHeight.current
									? height
									: props.scrollHeight || defaultHeight.current) + "px");
		...
		const getContentPosition = () => {
			if (contentRef.current) {
				...
				const top =
					...
					Math.max(parseFloat(style.top) || 0, 0);
				const bottom =
		...
		const setSize = () => {
			if (elementRef.current) {
				...
				const width =
					props.scrollWidth ||
					`${elementRef.current.offsetWidth || parentElement.offsetWidth}px`;
				const height =
					props.scrollHeight ||
					`${elementRef.current.offsetHeight || parentElement.offsetHeight}px`;
			...
			if (items) {
				...
				if (both) {
					...
					horizontal
						...
						: setProp("height", items, props.itemSize, contentPos.y);
				}
			}
		};
		...
		const setContentPosition = (pos) => {
			if (contentRef.current && !props.appendOnly) {
				...
				if (both) {
					setTransform(
						calculateTranslateVal(first.cols, props.itemSize[1]),
						calculateTranslateVal(first.rows, props.itemSize[0]),
					);
				} else {
		...
		const onScrollPositionChange = (event) => {
			...
			const calculateCurrentIndex = (_pos, _size) =>
				Math.floor(_pos / (_size || _pos));
			...
			if (both) {
				...
				if (
					!props.appendOnly ||
					(props.appendOnly && (isScrollDown || isScrollRight))
				) {
					const currentIndex = {
					...
					isRangeChanged =
						newFirst.rows !== firstState.rows ||
						newLast.rows !== lastState.rows ||
						newFirst.cols !== firstState.cols ||
						newLast.cols !== lastState.cols ||
						isItemRangeChanged.current;
			...
			if (isRangeChanged) {
				...
				if (props.lazy && isPageChanged(first)) {
					const newLazyLoadState = {
						first: props.step
							? Math.min(
									getPageByFirst(first) * props.step,
									props.items.length - props.step,
							  )
							: first,
						last: Math.min(
							props.step ? (getPageByFirst(first) + 1) * props.step : last,
							props.items.length,
						),
					};
					...
					const isLazyStateChanged =
						!lazyLoadState.current ||
						lazyLoadState.current.first !== newLazyLoadState.first ||
						lazyLoadState.current.last !== newLazyLoadState.last;

					isLazyStateChanged &&
						props.onLazyLoad &&
						props.onLazyLoad(newLazyLoadState);
			...
			if (props.delay) {
				...
				if (isPageChanged(firstState)) {
					if (!loadingState && props.showLoader) {
						...
						const changed =
							isRangeChanged ||
							(props.step ? isPageChanged(firstState) : false);
			...
			resizeTimeout.current = setTimeout(() => {
				if (elementRef.current) {
					...
					if (reinit) {
						...
						defaultContentWidth.current = DomHandler.getWidth(
							contentRef.current,
						);
						defaultContentHeight.current = DomHandler.getHeight(
							contentRef.current,
						);
					}
				}
		...
		const loadedItems = () => {
			...
			if (items && !loadingState) {
				if (both)
					return items
						.slice(props.appendOnly ? 0 : firstState.rows, lastState.rows)
						.map((item) =>
							props.columns
								? item
								: item.slice(
										props.appendOnly ? 0 : firstState.cols,
										lastState.cols,
								  ),
						);
				...
				if (props.loadingTemplate) {
					content = loaderArrState.map((_, index) => {
						return createLoaderItem(
							index,
							both && { numCols: numItemsInViewportState.cols },
						);
					});
