    ...
    React.forwardRef((props, ref) => {
        ...
        const position = () => {
            if (elementRef.current) {
                ...
                const sublistWidth = elementRef.current.offsetParent
                    ...
                    : DomHandler.getHiddenElementOuterWidth(elementRef.current);
                const itemOuterWidth = DomHandler.getOuterWidth(
                    parentItem.children[0],
                );
                const top =
                    parseInt(containerOffset.top, 10) +
        ...
        const createMenuItem = (processedItem, index) => {
            ...
            const contentProps = mergeProps(
                {
                    onClick: (event) =>
                        onItemClick(event, processedItem, index),
                    onMouseEnter: (event) =>
        ...
        const createItem = (processedItem, index) => {
            return getItemProp(processedItem, "separator")
                ? createSeparator(index)
                : createMenuItem(processedItem, index);
        };
