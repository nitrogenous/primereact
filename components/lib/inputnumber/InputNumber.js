...
export const InputNumber = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const _locale = props.locale || context?.locale || PrimeReact.locale;
        ...
        const inputMode =
            props.inputMode ||
            (props.mode === "decimal" && !props.minFractionDigits
                ? "numeric"
                : "decimal");
        ...
        const getDecimalSeparator = () => {
            return new Intl.NumberFormat(_locale, { useGrouping: false })
                .format(1.1)
                .trim()
            ...
            switch (event.which) {
                ...
                case 8:
                    ...
                    if (selectionStart === selectionEnd) {
                        const deleteChar = inputValue.charAt(
                            selectionStart - 1,
                        );
                        ...
                        if (isNumeralChar(deleteChar)) {
                            ...
                            if (_group.current.test(deleteChar)) {
                                ...
                                newValueStr = `${inputValue.slice(
                                    0,
                                    selectionStart - 1,
                                )}0${inputValue.slice(selectionStart)}`;
                                newValueStr =
                            ...
                            const { minusCharIndex, currencyCharIndex } =
                                getCharIndexes(inputValue);

                            if (minusCharIndex === currencyCharIndex - 1) {
                                newValueStr =
                                    inputValue.slice(0, minusCharIndex) +
                                    inputValue.slice(selectionStart);
                            }
                        }
                        ...
                        if (isNumeralChar(deleteChar)) {
                            ...
                            if (_group.current.test(deleteChar)) {
                                ...
                                newValueStr = `${inputValue.slice(
                                    0,
                                    selectionStart,
                                )}0${inputValue.slice(selectionStart + 1)}`;
                                newValueStr =
                                    parseValue(newValueStr) > 0
                                        ? newValueStr
                                        : "";
        ...
        const replaceDecimalSeparator = (val) => {
            if (isFloat(val)) {
                return val
                    .toString()
                    .replace(/\.(?=[^.]*$)/, _decimalSeparator.current);
            }
        ...
        const isFloat = (val) => {
            ...
            const parseVal = parseValue(formatter.format(val));
        ...
        const insert = (
            ...
            if (sign.isMinusSign) {
                ...
                if (
                    isNewMinusSign &&
                    (selectionStart === 0 ||
                        selectionStart === currencyCharIndex + 1)
                ) {
                    newValueStr = inputValue;

                    if (isNewMinusSign || selectionEnd !== 0) {
                        newValueStr = insertText(
                            inputValue,
                            text,
                            0,
                            selectionEnd,
                ...
                const maxFractionDigits =
                    numberFormat.current.resolvedOptions()
        ...
        const insertText = (value, text, start, end) => {
            ...
            if (textSplit.length === 2) {
                const decimalCharIndex = value
                    .slice(start, end)
                    .search(_decimal.current);
        ...
        const isNumeralChar = (char) => {
            if (
                char.length === 1 &&
                (_numeral.current.test(char) ||
                    _decimal.current.test(char) ||
                    _group.current.test(char) ||
                    _minusSign.current.test(char))
            ) {
                resetRegex();
        ...
        const updateInput = (value, insertedValueStr, operation, valueStr) => {
            ...
            if (currentLength === 0) {
                ...
                if (operation === "range-insert") {
                    ...
                    const startValueStr =
                        startValue !== null ? startValue.toString() : "";
                    const startExpr = startValueStr
                        .split("")
                        .join(`(${groupChar.current})?`);
                    const sRegex = new RegExp(startExpr, "g");

                    sRegex.test(newValue);

                    const tExpr = insertedValueStr
                        .split("")
                        .join(`(${groupChar.current})?`);
                    ...
                    if (
                        ...
                        if (insertedValueStr === "0") {
                            ...
                            newSelectionEnd =
                                ...
                                Number(
                                    isDecimalSign(value) ||
                                        isDecimalSign(insertedValueStr),
                                );
                        }
        ...
        const concatValues = (val1, val2) => {
            if (val1 && val2) {
                ...
                const newVal1 = replaceDecimalSeparator(val1)
                    .split(_decimal.current)[0]
                    .replace(_suffix.current, "")
                    .trim();

                return decimalCharIndex !== -1
                    ? newVal1 + val2.slice(decimalCharIndex)
                    : val1;
            }
        ...
        const createUpButton = () => {
            ...
            const icon = props.incrementButtonIcon || (
                <AngleUpIcon {...incrementIconProps} />
            );
        ...
        const createDownButton = () => {
            ...
            const icon = props.decrementButtonIcon || (
                <AngleDownIcon {...decrementIconProps} />
            );
        ...
        const dataProps = ObjectUtils.reduceKeys(
            otherProps,
            DomHandler.DATA_PROPS,
        );
        ...
        const rootProps = mergeProps(
            {
                ...
                className: classNames(
                    props.className,
                    cx("root", { focusedState, stacked, horizontal, vertical }),
                ),
                style: props.style,
            },
            otherProps,
