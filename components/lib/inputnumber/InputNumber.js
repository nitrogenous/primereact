...
export const InputNumber = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const constructParser = () => {
			...
			const numerals = [
				...new Intl.NumberFormat(_locale, { useGrouping: false }).format(
					9876543210,
				),
			].reverse();
		...
		const getMinusSignExpression = () => {
			...
			return new RegExp(
				`[${formatter.format(-1).trim().replace(_numeral.current, "")}]`,
				"g",
			);
		};
					...
					if (selectionStart === selectionEnd) {
						...
						if (isNumeralChar(deleteChar)) {
							...
							if (_group.current.test(deleteChar)) {
								...
								const insertedText =
									isDecimalMode() &&
									(props.minFractionDigits || 0) < decimalLength
										? ""
										: "0";
		...
		const insert = (
			...
			if (sign.isMinusSign) {
				...
				if (
					isNewMinusSign &&
					(selectionStart === 0 || selectionStart === currencyCharIndex + 1)
				) {
				...
				const maxFractionDigits =
					numberFormat.current.resolvedOptions().maximumFractionDigits;
				const operation =
				...
				if (operation === "range-insert") {
					...
					if (operation === "insert" || operation === "delete-back-single") {
						...
						if (insertedValueStr === "0") {
							...
							newSelectionEnd =
								newSelectionEnd +
								Number(isDecimalSign(value) || isDecimalSign(insertedValueStr));
						}
		...
		const onInputFocus = (event) => {
			...
			if (
				...
				const end =
					inputValue.length === 0 ? 0 : inputValue.length - suffixLength;
