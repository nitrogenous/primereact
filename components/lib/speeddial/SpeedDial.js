    ...
    React.forwardRef((inProps, ref) => {
        ...
        const onEnterKey = (event) => {
            ...
            const itemIndex = [...items].findIndex(
                (item) => item.id === focusedOptionIndex,
            );

            onItemClick(event, props.model[itemIndex]);
            onBlur(event);

            const buttonEl = DomHandler.findSingle(
                elementRef.current,
                "button",
            );
        ...
        const navigateNextItem = (event, index = null) => {
            const optionIndex = findNextOptionIndex(
        ...
        const navigatePrevItem = (event, index = null) => {
            const optionIndex = findPrevOptionIndex(
                index || focusedOptionIndex,
            );
        ...
        const findPrevOptionIndex = (index) => {
            ...
            const newIndex =
                index === -1
                    ...
                    : index;
            let matchedOptionIndex = filteredItems.findIndex(
                (link) => link.getAttribute("id") === newIndex,
            );
        ...
        const calculateTransitionDelay = (index) => {
            ...
            return (
                (visible ? index : length - index - 1) * props.transitionDelay
            );
        };
        ...
        useMountEffect(() => {
            if (props.type !== "linear") {
                const button = DomHandler.findSingle(
                    elementRef.current,
                    ".p-speeddial-button",
                );
                const firstItem = DomHandler.findSingle(
                    listRef.current,
                    ".p-speeddial-item",
                );
                ...
                if (button && firstItem) {
                    const wDiff = Math.abs(
                        button.offsetWidth - firstItem.offsetWidth,
                    );
                    const hDiff = Math.abs(
                        button.offsetHeight - firstItem.offsetHeight,
                    );
        ...
        const createItem = (item, index) => {
            ...
            const contentClassName = classNames("p-speeddial-action", {
                "p-disabled": disabled,
            });
            ...
            const menuItemProps = mergeProps(
                {
                    ...
                    className: cx("menuitem", {
                        active: isItemActive(`${idState}_${index}`),
                    }),
        ...
        const createButton = () => {
            const showIconVisible =
                (!visible && !!props.showIcon) || !props.hideIcon;
            const hideIconVisible = visible && !!props.hideIcon;
            const className = classNames(
                "p-speeddial-button p-button-rounded",
                {
                    "p-speeddial-rotate":
                        props.rotateAnimation && !props.hideIcon,
                },
            ...
            const icon = showIconVisible
                ...
                : hideIconVisible
                  ...
                  : null;
            const toggleIcon = IconUtils.getJSXIcon(icon, undefined, {
                props,
                visible,
            });
