	...
	React.forwardRef((inProps, ref) => {
		...
		const changeFocusedOptionIndex = (index) => {
			...
			const filteredItems = [...items].filter(
				(item) =>
					!DomHandler.hasClass(DomHandler.findSingle(item, "a"), "p-disabled"),
			);
		...
		const findPrevOptionIndex = (index) => {
			...
			const newIndex =
				index === -1 ? filteredItems[filteredItems.length - 1].id : index;
			let matchedOptionIndex = filteredItems.findIndex(
				(link) => link.getAttribute("id") === newIndex,
			);

			matchedOptionIndex =
				index === -1 ? filteredItems.length - 1 : matchedOptionIndex - 1;
