...
export const ListBox = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const filteredValue =
			(props.onFilterValueChange ? props.filterValue : filterValueState) || "";
		...
		const onOptionSelect = (event) => {
			...
			props.multiple
				? onOptionSelectMultiple(event.originalEvent, option)
				: onOptionSelectSingle(event.originalEvent, option);
		...
		const onOptionSelectSingle = (event, option) => {
			...
			let value = null;
			const metaSelection = optionTouched.current
				? false
		...
		const removeOption = (option) => {
			return props.value.filter(
				(val) =>
					!ObjectUtils.equals(val, getOptionValue(option), props.dataKey),
			);
		};
				...
				if (props.optionGroupLabel) {
					for (let i = 0; i < visibleOptions.length; i++) {
						const selectedOptionIndex = findOptionIndexInList(
							props.value,
							getOptionGroupChildren(visibleOptions[i]),
						);
		...
		const isSelected = (option) => {
			...
			return props.multiple && props.value
				? props.value.some((val) => ObjectUtils.equals(val, optionValue, key))
		...
		const filter = (option) => {
			...
			const optionLabel = getOptionLabel(option).toLocaleLowerCase(
				props.filterLocale,
			);
		...
		const createItem = (option, index, scrollerOptions = {}) => {
			...
			if (props.optionGroupLabel) {
				const groupContent = props.optionGroupTemplate
					? ObjectUtils.getJSXElement(props.optionGroupTemplate, option, index)
		...
		const createEmptyMessage = (emptyMessage, isFilter) => {
			...
			const message =
				ObjectUtils.getJSXElement(emptyMessage, props) ||
				...
				const virtualScrollerProps = {
					...
					...{
						...
						itemTemplate: (item, options) =>
							item && createItem(item, options.index, options),
