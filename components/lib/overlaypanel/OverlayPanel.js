...
export const OverlayPanel = React.forwardRef((inProps, ref) => {
	...
	const overlayPanelDisplayOrder = useDisplayOrder(
		"overlay-panel",
		visibleState,
	);
	...
	useGlobalOnEscapeKey({
		...
		priority: [
			ESC_KEY_HANDLING_PRIORITIES.OVERLAY_PANEL,
			overlayPanelDisplayOrder,
		],
	});
	...
	const align = () => {
		if (currentTargetRef.current && overlayRef.current) {
			...
			if (containerOffset.top < targetOffset.top) {
				...
				isUnstyled &&
					DomHandler.removeClass(overlayRef.current, "p-overlaypanel-flipped");
			}
		}
	};
