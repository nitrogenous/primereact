...
export const MultiSelect = React.memo(
	React.forwardRef((inProps, ref) => {
			...
			switch (event.which) {
				...
				case 9:
					if (overlayVisibleState) {
						const firstFocusableElement = DomHandler.getFirstFocusableElement(
							overlayRef.current,
						);
		...
		const isClearClicked = (event) => {
			return (
				DomHandler.getAttribute(event.target, "data-pc-section") === "clearicon"
			);
		};
				...
				if (
					...
					if (ObjectUtils.isArray(props.value)) {
						return props.value.reduce(
							(acc, value, index) =>
								acc + (index !== 0 ? ", " : "") + getLabelByValue(value),
							"",
						);
					} else {
						return "";
		...
		const triggerIcon = (
			<div {...triggerProps}>{props.loading ? loadingIcon : dropdownIcon}</div>
		);
