...
export const Tooltip = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const isContentEmpty = (target) => {
			return !(
				props.content ||
				getTargetOption(target, "tooltip") ||
				props.children
			);
		};
		...
		const isDisabled = (target) => {
			return (
				getTargetOption(target, "disabled") === "true" ||
				hasTargetOption(target, "disabled") ||
				props.disabled
			);
		};
		...
		const isAutoHide = () => {
			return (
				getTargetOption(currentTargetRef.current, "autohide") || props.autoHide
			);
		};

		const getTargetOption = (target, option) => {
			return hasTargetOption(target, `data-pr-${option}`)
				? target.getAttribute(`data-pr-${option}`)
				: null;
		};
		...
		const getMouseTrackPosition = (target) => {
			const top =
				getTargetOption(target, "mousetracktop") || props.mouseTrackTop;
			const left =
				getTargetOption(target, "mousetrackleft") || props.mouseTrackLeft;
		...
		const updateTooltipState = (position) => {
			updateText(currentTargetRef.current, () => {
				...
				const mouseTrackCheck =
					isMouseTrack(currentTargetRef.current) || position === "mouse";
		...
		const show = (e) => {
			...
			if (visibleState) {
				...
				const success = sendCallback(props.onBeforeShow, {
					originalEvent: e,
					target: currentTargetRef.current,
				});
		...
		const hide = (e) => {
			...
			if (visibleState) {
				const success = sendCallback(props.onBeforeHide, {
					originalEvent: e,
					target: currentTargetRef.current,
				});
		...
		const align = (target, coordinate, position) => {
			...
			if ((isMouseTrack(target) || currentPosition == "mouse") && coordinate) {
				...
				const { top: mouseTrackTop, left: mouseTrackLeft } =
		...
		const updateContainerPosition = (position) => {
			if (elementRef.current) {
				...
				if (position === "left")
					elementRef.current.style.left =
						parseFloat(style.left) - parseFloat(style.paddingLeft) * 2 + "px";
				else if (position === "top")
					elementRef.current.style.top =
						parseFloat(style.top) - parseFloat(style.paddingTop) * 2 + "px";
			}
		};
		...
		const bindTargetEvent = (target) => {
			if (target) {
				...
				showEvents.forEach((event) =>
					currentTarget?.addEventListener(event, show),
				);
				hideEvents.forEach((event) =>
					currentTarget?.addEventListener(event, hide),
				);
			}
		};
		...
		const unbindTargetEvent = (target) => {
			if (target) {
				...
				showEvents.forEach((event) =>
					currentTarget?.removeEventListener(event, show),
				);
				hideEvents.forEach((event) =>
					currentTarget?.removeEventListener(event, hide),
				);
			}
		};
		...
		const applyDelay = (delayProp, callback) => {
			...
			const delay =
				getTargetOption(currentTargetRef.current, delayProp.toLowerCase()) ||
				props[delayProp];
			...
			if (target) {
				if (DomHandler.isElement(target)) {
					...
					const setEvent = (target) => {
						const element = DomHandler.find(document, target);
		...
		useMountEffect(() => {
			if (
				visibleState &&
				currentTargetRef.current &&
				isDisabled(currentTargetRef.current)
			) {
				hide();
			}
		});
		...
		useUpdateEffect(() => {
			if (visibleState) {
				...
				const classname = getTargetOption(
					currentTargetRef.current,
					"classname",
				);
			...
			const rootProps = mergeProps(
				{
					...
					className: classNames(
						props.className,
						cx("root", { positionState, classNameState }),
					),
					style: props.style,
					role: "tooltip",
