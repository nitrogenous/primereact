...
export const Tooltip = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const isShowOnDisabled = (target) => {
            return (
                getTargetOption(target, "showondisabled") ||
                props.showOnDisabled
            );
        };

        const isAutoHide = () => {
            return (
                getTargetOption(currentTargetRef.current, "autohide") ||
                props.autoHide
            );
        };
        ...
        const getMouseTrackPosition = (target) => {
            ...
            const left =
                getTargetOption(target, "mousetrackleft") ||
                props.mouseTrackLeft;
        ...
        const updateText = (target, callback) => {
            if (textRef.current) {
                const content =
                    getTargetOption(target, "tooltip") || props.content;
        ...
        const updateTooltipState = (position) => {
            updateText(currentTargetRef.current, () => {
                ...
                const mouseTrackCheck =
                    isMouseTrack(currentTargetRef.current) ||
                    position === "mouse";

                if (
                    (mouseTrackCheck && !containerSize.current) ||
                    mouseTrackCheck
                ) {
