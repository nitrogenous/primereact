	...
	React.forwardRef((inProps, ref) => {
		...
		const formatValue = (value) => {
			if (ObjectUtils.isNotEmpty(value)) {
				if (typeof value === "string") {
					...
					return resolvedFieldData !== null && resolvedFieldData !== undefined
						? resolvedFieldData
						: value;
				} else {
					return value;
				...
				switch (event.which) {
					...
					case 40:
						if (highlightItem) {
							...
							if (
								DomHandler.getAttribute(highlightItem, "data-pc-section") ===
								"itemgroup"
							) {
		...
		const findOptionIndex = (option) => {
			return props.suggestions
				? props.suggestions.findIndex((s) => ObjectUtils.equals(s, option))
				: -1;
		};
		...
		const getOptionGroupLabel = (optionGroup) => {
			return props.optionGroupLabel
				? ObjectUtils.resolveFieldData(optionGroup, props.optionGroupLabel)
				: optionGroup;
		};
		...
		useUpdateEffect(() => {
			if (searchingState) {
				ObjectUtils.isNotEmpty(props.suggestions) || props.showEmptyMessage
					? show()
					: hide();
				setSearchingState(false);
			}
			...
			if (ObjectUtils.isNotEmpty(props.value)) {
				return props.value.map((val, index) => {
					...
					const removeTokenIcon =
						!props.disabled &&
						IconUtils.getJSXIcon(icon, { ...removeTokenIconProps }, { props });
