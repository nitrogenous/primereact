...
export const AutoComplete = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const { ptm, cx, sx, isUnstyled } =
            AutoCompleteBase.setMetaData(metaData);
        ...
        const isInputClicked = (event) => {
            return props.multiple
                ? event.target === multiContainerRef.current ||
                      multiContainerRef.current.contains(event.target)
        ...
        const updateModel = (event, value) => {
            ...
            if (
                selectedItem &&
                ObjectUtils.deepEquals(selectedItem.current, value)
            ) {
                return;
            }
        ...
        const onInputKeyDown = (event) => {
            if (overlayVisibleState) {
                ...
                switch (event.which) {
                    ...
                    case 40:
                        if (highlightItem) {
                            const nextElement = findNextItem(highlightItem);
                            ...
                            highlightItem = DomHandler.findSingle(
                                overlayRef.current,
                                "li",
                            );
                    ...
                    case 38:
                        if (highlightItem) {
                            const previousElement = findPrevItem(highlightItem);
        ...
        const selectHighlightItem = (event, item) => {
            if (props.optionGroupLabel) {
                ...
                selectItem(
                    event,
                    getOptionGroupChildren(optionGroup)[item.dataset.index],
                );
            } else {
                selectItem(
                    event,
                    props.suggestions[item.getAttribute("index")],
                );
            }
        };
        ...
        const forceItemSelection = (event) => {
            ...
            const item = (props.suggestions || []).find((it) => {
                const value = props.field
                    ? ObjectUtils.resolveFieldData(it, props.field)
                    : it;
        ...
        const isSelected = (val) => {
            return props.value
                ? props.value.some((v) => ObjectUtils.equals(v, val))
                : false;
        };

        const findOptionIndex = (option) => {
            return props.suggestions
                ? props.suggestions.findIndex((s) =>
                      ObjectUtils.equals(s, option),
                  )
                : -1;
        };
        ...
        const isAllowMoreValues = () => {
            return (
                ...
                props.value.length < props.selectionLimit
            );
        };
        ...
        useUpdateEffect(() => {
            if (searchingState) {
                ObjectUtils.isNotEmpty(props.suggestions) ||
        ...
        const createChips = () => {
            if (ObjectUtils.isNotEmpty(props.value)) {
                return props.value.map((val, index) => {
                    ...
                    const icon = props.removeTokenIcon || (
                        <TimesCircleIcon {...removeTokenIconProps} />
                    );
