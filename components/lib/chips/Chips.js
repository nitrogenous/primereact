...
export const Chips = React.memo(
	React.forwardRef((inProps, ref) => {
			...
			switch (event.key) {
				...
				default:
					...
					if (isMaxedOut()) {
						...
						if (
							event.key === props.separator ||
							(DomHandler.isAndroid() && event.which === 229)
						) {
							addItem(event, inputValue, true);
						}
					}
			...
			if (props.separator) {
				...
				if (pastedData) {
					...
					pastedValues = pastedValues.filter(
						(val) =>
							(props.allowDuplicate || values.indexOf(val) === -1) &&
							val.trim().length,
					);
		...
		const createRemoveIcon = (value, index) => {
			...
			const removeIcon = IconUtils.getJSXIcon(
				icon,
				{ ...iconProps },
				{ props },
			);
