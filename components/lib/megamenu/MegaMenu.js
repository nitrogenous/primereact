...
export const MegaMenu = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const isMobileMode = useMatchMedia(
            `screen and (max-width: ${props.breakpoint})`,
            !!props.breakpoint,
        );
        ...
        const onLeafClick = (event) => {
            ...
            if (selected) {
                ...
                if (grouped) {
                    ...
                    const rootProcessedItemIndex = activeItemState
                        ? activeItemState.index
                        : -1;
                    const rootProcessedItemKey = activeItemState
                        ? activeItemState.key
                        : "";
        ...
        useUpdateEffect(() => {
            const currentPanel = DomHandler.findSingle(
                elementRef.current,
        ...
        useUpdateEffect(() => {
            const _focusedItemId = ObjectUtils.isNotEmpty(focusedItemInfo.key)
                ? `${idState}_${focusedItemInfo.key}`
                : null;
        ...
        useUpdateEffect(() => {
            const processedItem = ObjectUtils.isNotEmpty(activeItemState)
                ? activeItemState
                : null;
        ...
        const onKeyDown = (event) => {
            ...
            switch (event.code) {
                ...
                default:
                    if (
                        !metaKey &&
                        ObjectUtils.isPrintableCharacter(event.key)
                    ) {
        ...
        const onArrowDownKey = (event) => {
            ...
            if (horizontal) {
                ...
                if (
                    ...
                    activeItemState.key === focusedItemInfo.key
                ) {
                    _focusedItemInfo = {
                        index: -1,
                        key: "",
                    ...
                    const processedItem = findVisibleItem(
                        focusedItemInfo.index,
                    );
                    const grouped = isProccessedItemGroup(processedItem);
        ...
        const onArrowUpKey = (event) => {
            ...
            if (event.altKey && horizontal) {
                ...
                const itemIndex =
                    focusedItemInfo.index !== -1
                        ? findPrevItemIndex(focusedItemInfo.index)
        ...
        const onArrowRightKey = (event) => {
            ...
            if (grouped) {
                if (vertical) {
                    if (
                        ObjectUtils.isNotEmpty(activeItemState) &&
                        activeItemState.key === processedItem.key
                    ) {
                        setFocusedItemInfo({
                            index: -1,
                            key: "",
        ...
        const isItemMatched = (processedItem) => {
            ...
            return (
                ...
                label
                    .toLocaleLowerCase()
                    .startsWith(searchValue.current.toLocaleLowerCase())
            );
        };
        ...
        const isSelected = (processedItem) => {
            return ObjectUtils.isNotEmpty(activeItemState)
                ? activeItemState.key === processedItem.key
                : false;
        };

        const findFirstItemIndex = () => {
            return visibleItems.findIndex((processedItem) =>
                isValidItem(processedItem),
            );
        };

        const findLastItemIndex = () => {
            return ObjectUtils.findLastIndex(visibleItems, (processedItem) =>
                isValidItem(processedItem),
            );
        };
            ...
            const matchedItemIndex =
                index > 0
                    ? ObjectUtils.findLastIndex(
                          ...
                          (processedItem) => isValidItem(processedItem),
                      )
                    : -1;
        ...
        const findVisibleItem = (index) => {
            return ObjectUtils.isNotEmpty(visibleItems)
                ? visibleItems[index]
                : null;
        };
        ...
        const searchItems = (event, char) => {
            ...
            if (focusedItemInfo.index !== -1) {
                ...
                itemIndex = visibleItems.findIndex((processedItem) =>
                    isItemMatched(processedItem),
                );
            }
        ...
        const changeFocusedItemInfo = (index) => {
            const processedItem = findVisibleItem(index);
            const key = ObjectUtils.isNotEmpty(processedItem)
                ? processedItem.key
                : "";
                ...
                const newItem = {
                    ...
                    columnIndex:
                        columnIndex !== undefined
                            ? columnIndex
                            : parent && parent.columnIndex !== undefined
                              ? parent.columnIndex
                              : index,
                };
        ...
        const createSubmenuIcon = (item) => {
            if (item.items) {
                ...
                const icon = vertical
                    ? props.submenuIcon || (
                          <AngleRightIcon {...submenuIconProps} />
                      )
                    : props.submenuIcon || (
                          <AngleDownIcon {...submenuIconProps} />
                      );
            ...
            const submenuHeaderProps = mergeProps(
                {
                    ...
                    className: classNames(
                        submenu.className,
                        cx("submenuHeader", { disabled: isDisabled }),
                    ),
                    style: submenu.style,
                    role: "presentation",
                    "data-p-disabled": isDisabled,
        ...
        const createCategory = (processedItem, index) => {
            ...
            const label = category.label && (
                <span {...labelProps}>{category.label}</span>
            );
            ...
            const menuItemProps = mergeProps(
                {
                    ...
                    "aria-setsize": getAriaSetSize(),
                    "aria-posinset": getAriaPosInset(index),
                    "aria-expanded": isItemGroup(processedItem)
                        ? isItemActive(processedItem)
                        : undefined,
                    "aria-haspopup": isItemGroup(processedItem)
                        ? "menu"
                        : undefined,
            ...
            const menuButtonProps = mergeProps(
                {
                    ...
                    role: "button",
                    "aria-haspopup":
                        props.model && props.model.length > 0 ? true : false,
