	...
	React.forwardRef((inProps, ref) => {
		...
		const toggle = (event) => {
			...
			if (mobileActiveState) {
				...
				ZIndexUtils.set(
					...
					(context && context.zIndex["menu"]) || PrimeReact.zIndex["menu"],
				);
				setTimeout(() => {
					show();
				}, 1);
					...
					newItem["items"] =
						level === 0 && item.items && item.items.length > 0
							? item.items.map((_items, _index) =>
									createProcessedItems(_items, level + 1, newItem, key, _index),
							  )
		...
		const createSubmenuIcon = (item) => {
			if (item.items) {
				...
				const icon = vertical
					? props.submenuIcon || <AngleRightIcon {...submenuIconProps} />
					: props.submenuIcon || <AngleDownIcon {...submenuIconProps} />;
		...
		const createColumn = (processedItem, processedColumn, index) => {
			...
			const display =
				activeItemState && activeItemState.item === category ? "block" : "none";
