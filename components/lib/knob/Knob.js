...
export const Knob = React.memo(
	React.forwardRef((inProps, ref) => {
		...
		const [bindWindowMouseMoveListener, unbindWindowMouseMoveListener] =
			useEventListener({
				target: "window",
				type: "mousemove",
				listener: (event) => {
		...
		const [bindWindowMouseUpListener, unbindWindowMouseUpListener] =
			useEventListener({
				target: "window",
				type: "mouseup",
				listener: (event) => {
		...
		const [bindWindowTouchMoveListener, unbindWindowTouchMoveListener] =
			useEventListener({
				target: "window",
				type: "touchmove",
				listener: (event) => {
		...
		const [bindWindowTouchEndListener, unbindWindowTouchEndListener] =
			useEventListener({
				target: "window",
				type: "touchend",
				listener: () => {
		...
		const zeroRadians = () =>
			mapRange(
				props.min > 0 && props.max > 0 ? props.min : 0,
				props.min,
				props.max,
				minRadians,
				maxRadians,
		...
		const valueRadians = () =>
			mapRange(props.value, props.min, props.max, minRadians, maxRadians);
		...
		const largeArc = () =>
			Math.abs(zeroRadians() - valueRadians()) < Math.PI ? 0 : 1;
		...
		const valueToDisplay = () =>
			props.valueTemplate.replace("{value}", props.value.toString());
		...
		const updateModel = (angle, start) => {
			...
			if (props.onChange) {
				props.onChange({
					value:
						Math.round((mappedValue - props.min) / props.step) * props.step +
						props.min,
				});
			}
		};
		...
		const text = props.showValue && (
			<text {...labelProps}>{valueToDisplay()}</text>
		);
