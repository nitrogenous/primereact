...
export function LazyVirtualScrollDoc(props) {
	...
	const loadDemoData = () => {
		cars = Array.from({ length: 100000 }).map((_, i) =>
	...
	return (
		<>
			<DocSectionText {...props}>
				<p>
					When lazy loading is enabled via the <i>virtualScrollerOptions</i>,
					data is fetched on demand during scrolling instead of preload.
				</p>
				<p>
					In sample below, an in-memory list and timeout is used to mimic
					fetching from a remote datasource. The <i>virtualCars</i> is an empty
					array that is populated on scroll.
				</p>
			</DocSectionText>
