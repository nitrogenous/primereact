...
export function ListDoc(props) {
    ...
    const onFilter = (event) => {
        ...
        if (icons) {
            setFilteredIcons(
                icons.filter(
                    (it) => it.icon.tags[0].indexOf(event.target.value) !== -1,
                ),
            );
        }
    };
    ...
    return (
        <>
            <DocSectionText {...props}>
                <p>
                    Here is the full list of PrimeIcons. More icons will be
                    added periodically and you may also{" "}
            ...
            <InputText
                className="w-full p-3 mt-3 mb-4"
