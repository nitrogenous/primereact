		...
		const createContent = (value, isLinkableOption, deprecated) => {
				...
				return splitedValues.map((sValue, i) => {
					if (sValue.includes(name)) {
						...
						const apiId =
							name === val
								? `api.${name}`
								: `api.${name}.${val === `${name}Props` ? "props" : val}`;

						return (
							<React.Fragment key={i}>
								{i !== 0 ? "|" : ""}
								<Link
									href={router.basePath + router.pathname + `#${apiId}`}
									target="_self"
								>
			...
			return isLinkableOption ? (
				...
				>
					{val}
					<Link
						href={router.basePath + router.pathname + `#${id + "." + val}`}
						target="_self"
					>
						<a
											...
											k !== "deprecated" && (
												<td key={index}>
													{k === "parameters" ? (
														...
														<div
															className={classNames("doc-option-default", {
																"doc-option-dark": appContentContext.darkTheme,
																"doc-option-light":
														...
														<div
															className={classNames("doc-option-returnType", {
																"doc-option-dark": appContentContext.darkTheme,
																"doc-option-light":
																	!appContentContext.darkTheme,
															})}
