    ...
    const docs = doc.reduce((cDocs, name) => {
        ...
        if (mod) {
            const addToChildDoc = (childDoc, componentName) => {
                if (
                    ObjectUtils.isNotEmpty(mod.events) &&
                    ObjectUtils.isNotEmpty(mod.events.values) &&
                    !isExcludedAll("events")
                ) {
                    const eMap = {
                    ...
                    Object.entries(mod.events.values).forEach(
                        ([eKey, eValue]) => {
                            const [id, label] = [
                                `api.${componentName}.${eKey}`,
                                eKey,
                            ];
                            ...
                            eMap.children.push({
                                ...
                                component: (inProps) => (
                                    <DocApiTable
                                        ...
                                        description={
                                            <>
                                                {eValue.description} See{" "}
                                                <i>{eValue.relatedProp}</i>.
                                            </>
                                        }
                                        {...inProps}
                                    />
                ...
                if (
                    ...
                    ObjectUtils.isNotEmpty(mod.interfaces.values) &&
                    !isExcludedAll("interfaces")
                ) {
                    const iMap = {
                    ...
                    Object.entries(mod.interfaces.values).forEach(
                        ([iKey, iValue]) => {
                            const [id, label] = [
                                `api.${componentName}.${iKey}`,
                                iKey,
                            ];
                ...
                if (
                    ObjectUtils.isNotEmpty(mod.types) &&
                    ObjectUtils.isNotEmpty(mod.types.values) &&
                    !isExcludedAll("types")
                ) {
                    const tMap = {
                    ...
                    Object.entries(mod.types.values).forEach(
                        ([tKey, tValue]) => {
                            const [id, label] = [
                                `api.${componentName}.${tKey}`,
                                tKey,
                            ];
            ...
            if (splitedName.length === 3) {
                ...
                if (type === "functions") {
                    ...
                    if (
                        ...
                        Object.entries(mod.interfaces.values).forEach(
                            ([iKey, iValue]) => {
                                if (types.includes(iKey)) {
                                    ...
                                    if (
                                        ObjectUtils.isNotEmpty(iValue.callbacks)
                                    ) {
                                        tMap.children.push({
                                            id: `${id}.callbacks`,
                                            label: "Callbacks",
                        ...
                        ObjectUtils.isNotEmpty(iMap.children) &&
                            fMap.children.push(iMap);
                    }
                ...
                mod.components &&
                    Object.entries(mod.components).forEach(([cKey, cValue]) => {
                        ...
                        if (
                            ObjectUtils.isNotEmpty(cValue.props) &&
                            ObjectUtils.isNotEmpty(cValue.props.values) &&
                            !isExcludedAll("props")
                        ) {
                            ...
                            cMap.children.push({
                                ...
                                component: (inProps) => (
                                    <DocApiTable
                                        name={cKey}
                                        data={cValue.props.values}
                                        description={cValue.props.description}
                                        {...inProps}
                                    />
                                ),
                            });
                        ...
                        if (
                            ObjectUtils.isNotEmpty(cValue.callbacks) &&
                            ObjectUtils.isNotEmpty(cValue.callbacks.values) &&
                            !isExcludedAll("callbacks")
                        ) {
                            const [id, label] = [
                                `api.${cKey}.callbacks`,
                                "Callbacks",
                            ...
                            cMap.children.push({
                                ...
                                component: (inProps) => (
                                    <DocApiTable
                                        name={cKey}
                                        data={cValue.callbacks.values}
                                        description={
                                            cValue.callbacks.description
                                        }
                                        {...inProps}
                                    />
                                ),
                        ...
                        if (
                            ObjectUtils.isNotEmpty(cValue.methods) &&
                            ObjectUtils.isNotEmpty(cValue.methods.values) &&
                            !isExcludedAll("methods")
                        ) {
                            const [id, label] = [
                                `api.${cKey}.methods`,
                                "Methods",
                            ...
                            cMap.children.push({
                                ...
                                component: (inProps) => (
                                    <DocApiTable
                                        name={cKey}
                                        data={cValue.methods.values}
                                        description={cValue.methods.description}
                                        {...inProps}
                                    />
                                ),
                            });
                ...
                mod.model &&
                    Object.entries(mod.model).forEach(([mKey, mValue]) => {
                        ...
                        if (
                            ObjectUtils.isNotEmpty(mValue.props) &&
                            ObjectUtils.isNotEmpty(mValue.props.values) &&
                            !isExcludedAll("props")
                        ) {
                            ...
                            mMap.children.push({
                                ...
                                component: (inProps) => (
                                    <DocApiTable
                                        name={mKey}
                                        data={mValue.props.values}
                                        description={mValue.props.description}
                                        {...inProps}
                                    />
                                ),
                            });
        ...
        <>
            <div className="doc-main">
                <div className="doc-intro">
                    <h1>{header} API</h1>
                    <p>
                        API defines helper props, events and others for the
                        PrimeReact {header} module.
                    </p>
                </div>
