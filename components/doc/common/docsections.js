...
export function DocSections({ docs }) {
	const getPTOption = (name) => {
		...
		let values =
			APIDocs[key]?.interfaces?.values[`${name}PassThroughOptions`] || null;

		if (!values) {
			for (const parentKey in APIDocs) {
				if (
					APIDocs[parentKey]?.interfaces?.values[`${name}PassThroughOptions`]
				) {
					values =
						APIDocs[parentKey]?.interfaces?.values[
