...
import {
    ...
    ObjectUtils,
    classNames,
} from "@/components/lib/utils/Utils";
...
export function DocSectionNav({ docs = [] }) {
    ...
    const [bindDocumentScrollListener] = useEventListener({
        ...
        listener: (event) => {
            if (!isScrollBlocked.current) {
                const labels = DomHandler.find(
                    event.target,
                    ":is(h1,h2,h3).doc-section-label",
                );
    ...
    const getThreshold = (label) => {
        if (!topbarHeight.current) {
            const topbar = DomHandler.findSingle(
                document.body,
                ".layout-topbar",
            );
        ...
        return (
            <li
                ...
                className={classNames("navbar-item", {
                    "active-navbar-item": activeId === id,
                })}
            >
                ...
                <ul>
                    {level !== 1 &&
                        children &&
                        children.map((child) => createItem(child, 1))}
                </ul>
            </li>
        );
    };
