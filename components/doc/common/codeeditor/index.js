...
const useCodeSandbox = (props) => {
    const getSandboxParameters = (sourceType) => {
        ...
        const { files, dependencies, sourceFileName } = getVite(
            { dependencies: sandboxDependencies, ...props },
            sourceType,
        );
    ...
    return (sourceType, errorCallback) => {
        ...
        fetch("https://codesandbox.io/api/v1/sandboxes/define?json=1", {
            ...
            .then((data) =>
                window.open(
                    `https://codesandbox.io/s/${data.sandbox_id}`,
                    "_blank",
                ),
            );
    };
...
export const useStackBlitz = (props) => {
    ...
    return (sourceType, errorCallback) => {
        ...
        Object.entries(stackBlitzParameters.files).forEach(
            ([k, v]) =>
                (files[`${k}`] =
