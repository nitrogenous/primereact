...
export default function Layout({ children }) {
	...
	const { theme, darkMode, newsActive, changeTheme } =
	...
	const toggleDarkMode = () => {
		...
		if (darkMode) {
			...
			if (theme.includes("light") && theme !== "fluent-light")
	...
	return (
		<div className={wrapperClassName} data-p-theme={theme}>
			...
			<Topbar
				showConfigurator
				showMenuButton
				onMenuButtonClick={() => setSidebarActive(true)}
				onConfigButtonClick={() => setConfigActive(true)}
				onDarkSwitchClick={toggleDarkMode}
			/>
			<div
				className={classNames("layout-mask", {
