...
const TemplateFeatures = ({ featuresData, displayType }) => {
            ...
            <div className="template-features-horizontal-wrapper">
                <div className="template-features-horizontal">
                    {featuresData.map((data, i) => (
                        <div
                            key={i}
                            className="template-features-horizontal-card"
                        >
                            <div className="template-features-horizontal-card-top">
                                {ObjectUtils.getJSXElement(data.template) || (
                                    <img src={data.src} alt={data.title} />
                                )}
                            </div>
                            <div className="template-features-horizontal-card-bottom">
                                <h5 className="template-features-horizontal-card-bottom-title">
                                    {data.title}
                                </h5>
                                <p className="template-features-horizontal-card-bottom-description">
                                    {data.description}
                                </p>
                            </div>
                        </div>
    ...
    const renderVerticalFeatures = () => {
        const firstColumnData = featuresData.slice(
            0,
            Math.ceil(featuresData.length / 2),
        );
        const secondColumnData = featuresData.slice(
            Math.ceil(featuresData.length / 2),
        );
                ...
                <div className="template-features-vertical">
                    {Array(2)
                        ...
                        .map((_, i) => (
                            <div
                                key={i}
                                className="template-features-vertical-col"
                            >
                                {(i === 0
                                    ? firstColumnData
                                    : secondColumnData
