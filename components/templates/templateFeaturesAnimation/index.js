...
const TemplateFeaturesAnimation = ({
	featuresData,
	title,
	animationSeconds = 5000,
}) => {
	const animationRef = useRef(null);
	const { selectedID, handleClick } = useAnimatedFeatures(
		animationRef,
		featuresData.length,
		animationSeconds,
	);
				...
				<div className="template-features-animation-left">
					{featuresData.map((data, i) => {
						...
						return (
							<div
								key={i}
								className={`template-features-animation-left-card ${
									selectedID === data.id
										? "template-features-animation-left-card-active"
										: ""
								}`}
				...
				<div className="template-features-animation-right">
					{featuresData[selectedID - 1]?.type === "inline-animation" ? (
						<TemplateFeaturesAnimationInline
							inlineFeaturesData={
								featuresData[selectedID - 1]?.inlineFeaturesData
							}
							parentHandleClick={handleClick}
							parentID={selectedID}
							inlineSeconds={
								animationSeconds /
								featuresData[selectedID - 1]?.inlineFeaturesData.length
							}
						/>
					) : (
						<img
